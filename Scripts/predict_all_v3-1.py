...
out_root = "/data/student/Fengjunming/SDXL_ControlNet/results/out_preds_sd21_v3_1"
SIZE = 768
...
def main():
    ...
    if args.ctrl_dir:
        ...
        base_ctrl = os.path.join(ctrl_root, args.mode, args.name if args.name else "v3_1_hed")
    ...
    base_out = os.path.join(out_root, args.mode, args.name if args.name else "v3_1_hed")
    ...
    with open(args.csv) as f:
        for i, row in enumerate(csv.DictReader(f)):
            ...
            cond = hed(src_pil)
            cond.save(os.path.join(cond_dir, f"{idx}_cond_hed.png"))
            img = pipe(
                prompt=args.prompt,
