...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + Dual ControlNet 推理参数 (v8-1 双路架构 - 所有模式)\n")
    f.write("="*70 + "\n")
    ...
    f.write("【v8-1架构】双路 ControlNet: Scribble(Vessel) + Tile\n")
    if is_cffa:
...
print(f"  【v8-1架构】双路 ControlNet (Scribble + Tile)")
...
print(f"  【v8-1架构】双路 ControlNet: Scribble(Vessel) + Tile")
if is_cffa:
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        if is_cffa:
            ...
            # 2. Tile 输入：预处理后的图像（与训练时一致）
            if args.mode == "cf2octa":
                # cf2octa: 使用绿色通道+取反后的图像（训练集是灰度图）
                cond_tile = src_preprocessed.resize((SIZE, SIZE))
            else:  # octa2cf
...
print(f"  架构: 双路 ControlNet (Scribble + Tile) - v8-1")
print(f"  数据集: {'CF-FA' if is_cffa else 'CF-OCTA'}")
...
if len(valid_metrics) > 0:
    ...
    with open(log_path, "a") as f:
        ...
        f.write("    【v8-1架构】双路 ControlNet (Scribble + Tile)\n")
