"""
SD 1.5 ControlNet 推理脚本 - v9-2

【v9-2 更新】CF_OCT 配准方案升级（参考 main 方法）
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        # v9-2: ControlNet 预处理 - 支持 CF-FA、CF-OCTA 和 CF_OCT
        # 【v9-2 修复】与训练脚本一致：先 resize_with_padding，再提取血管
        
        # 1. 先 resize_with_padding 条件图（Tile 输入）
        cond_tile_np, _, _, _ = resize_with_padding(
            np.array(src_img_original),
            target_size=(SIZE, SIZE),
            interpolation=cv2.INTER_CUBIC
        )
        cond_tile = Image.fromarray(cond_tile_np)
        
        # 2. 从 Tile 输入提取 Scribble 条件图（Vessel 血管图）
        if is_cffa:
            # CF-FA 模式：双路 ControlNet (Scribble + Tile)
            ...
            cond_scribble = extract_vessel_hessian(
                cond_tile,  # 【v9-2 修复】从 resize_with_padding 后的图像提取
            ...
            # Scribble 输入：Vessel 血管图（应用Frangi滤波）
        ...
        if is_cfoct:
            ...
            # 检测黑色像素（包括配准插值产生的接近黑色像素）
            # 阈值=2 可以捕获纯黑(0)和插值产生的接近黑色像素(1-2)
            black_mask = np.all(cond_tile_np <= 2, axis=2)  # (H, W) boolean
