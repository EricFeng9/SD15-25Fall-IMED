...
【使用方法】
训练: python train_controlnet_sd15_v5-1.py --mode cf2octa --name sd15_v5-1 --max_steps 8000
恢复: python train_controlnet_sd15_v5-1.py --mode cf2octa --name sd15_v5-1 --resume_from /path/to/step_6000
...
def run_inference_test(fixed_sample, step_dir, step_num, fixed_seed=42):
    ...
    参数:
        fixed_sample: (cond_hed, cond_orig, tgt, cond_path, tgt_path) - 固定的测试样本
        step_dir: checkpoint 保存目录
        step_num: 当前步数
        fixed_seed: 固定的随机种子（用于对比不同step的训练效果）
    """
    print(f"\n{'='*70}")
    print(f"运行推理测试 (step {step_num}) - 双路 ControlNet")
    print(f"{'='*70}")
    
    cond_hed, cond_orig, tgt, cond_path, tgt_path = fixed_sample
    ...
    cond_orig_save = (cond_orig.cpu().float().permute(1, 2, 0).numpy() * 255).clip(0, 255).astype(np.uint8)
    Image.fromarray(cond_hed_save).save(os.path.join(infer_dir, "condition_hed.png"))
    Image.fromarray(cond_orig_save).save(os.path.join(infer_dir, "condition_original.png"))
    
    # 构建推理 pipeline（双路 ControlNet）
    controlnet.eval()
    ...
    cond_orig_pil = Image.fromarray(cond_orig_save)
    ...
    with torch.no_grad():
        output = pipe(
            ...
            image=[cond_hed_pil, cond_orig_pil],  # 两个条件图
...
def main():
    ...
    print(f"  条件图: {fixed_sample[3]}")  # cond_path
    print(f"  目标图: {fixed_sample[4]}\n")  # tgt_path
    ...
    if args.resume_from:
        ...
        controlnet_hed = ControlNetModel.from_pretrained(
            os.path.join(args.resume_from, "controlnet_hed"), 
            local_files_only=True
        ).to(device)
        controlnet_tile = ControlNetModel.from_pretrained(
            os.path.join(args.resume_from, "controlnet_tile"), 
            local_files_only=True
        ).to(device)
        ...
        print(f"✓ 已加载双路 ControlNet checkpoint")
    else:
        # 从预训练模型开始（FP32）
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            if global_step % 100 == 0:
                ...
                msg = (f"[SD15-v5-5] step {global_step}/{max_steps} | "
