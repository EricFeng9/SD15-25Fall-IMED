# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v8-1

【基于】train_controlnet_sd15_v8.py
【模型】Stable Diffusion 1.5 (512×512) + 双路 ControlNet (Scribble + Tile)
【数据集】支持两种数据集
  - CF-OCTA: 配准数据集 (HED边缘 + Tile，Scribble权重=0)
  - CF-FA: CFFA 数据集 (Vessel血管 + Tile，Scribble权重>0)

【核心特点】
- 双路 ControlNet 架构（血管结构 + 原图细节）
  * ControlNet-Scribble: 血管结构引导（CF-FA模式）或HED边缘（CF-OCTA模式，权重=0）
  * ControlNet-Tile: 原图细节保留（纹理、强度、颜色）
- SD 1.5 原生支持 512×512 分辨率
- 智能权重控制：CF-OCTA模式自动禁用Scribble
- **【v8-1新增】多损失函数训练 + 动态权重 + 学习率调度**
- FP32 训练

【v8-1 更新】✨
1. **血管结构损失 (Vessel Loss)**
   - 使用 Sobel 边缘检测 + Soft Dice Loss（完全可微）
   - 直接约束血管结构，防止细节过度平滑
   - 权重: step<3000 为 0.05，step≥3000 为 0.5

2. **动态损失权重调度**
   - Phase 1 (step<3000): mse=1.0, msssim=0.1, vessel=0.05
   - Phase 2 (step≥3000): mse=0.3, msssim=0.05, vessel=0.5
   - 后期更关注细节而非像素匹配
...
【使用方法】
# CF-FA 训练（双路Scribble+Tile模式，带血管损失）
python train_controlnet_sd15_v8.py --mode cf2fa --name sd15_v8-1_cffa --max_steps 8000 --scribble_scale 0.8

参数调节:
  --scribble_scale: Scribble ControlNet 强度 (默认 0.8，CF-OCTA自动设为0)
  --tile_scale: Tile ControlNet 强度 (默认 1.0)
  --msssim_lambda: MS-SSIM 损失权重 (默认 0.1，在 step≥3000 自动降到 0.05)
  --vessel_lambda: Vessel Loss 权重 (默认 0.05→0.5，根据step自动调整)
'''
...
from data_loader_cffa_v2 import PairCSV_CFFA, SIZE as SIZE_CFFA
...
# ============ v8-1 新增：血管结构损失函数 ============
def compute_vessel_loss(pred_imgs, gt_imgs, smooth=1.0):
    """
    血管结构损失 - 使用 Sobel 边缘检测 + Soft Dice Loss
    
    参数:
        pred_imgs: 预测图像 (B, 3, H, W)，范围 [-1, 1]
        gt_imgs: 目标图像 (B, 3, H, W)，范围 [-1, 1]
        smooth: Dice Loss 平滑项，防止除零
    
    返回:
        vessel_loss: Soft Dice Loss (标量)
    ...
    # 2. 转换为灰度图（取平均）
    pred_gray = pred_01.mean(dim=1, keepdim=True)  # (B, 1, H, W)
    gt_gray = gt_01.mean(dim=1, keepdim=True)
    
    # 3. Sobel 边缘检测（可微！）
    # 手动实现 Sobel 卷积核（避免依赖 kornia）
    sobel_x = torch.tensor([
        [[-1, 0, 1],
         [-2, 0, 2],
         [-1, 0, 1]]
    ], dtype=pred_gray.dtype, device=pred_gray.device).unsqueeze(0)  # (1,1,3,3)
    ...
    # 对预测图应用 Sobel
    pred_grad_x = torch.nn.functional.conv2d(pred_gray, sobel_x, padding=1)
    pred_grad_y = torch.nn.functional.conv2d(pred_gray, sobel_y, padding=1)
    pred_edges = torch.sqrt(pred_grad_x**2 + pred_grad_y**2 + 1e-8)  # (B,1,H,W)
    
    # 对GT图应用 Sobel
    gt_grad_x = torch.nn.functional.conv2d(gt_gray, sobel_x, padding=1)
    gt_grad_y = torch.nn.functional.conv2d(gt_gray, sobel_y, padding=1)
    gt_edges = torch.sqrt(gt_grad_x**2 + gt_grad_y**2 + 1e-8)  # (B,1,H,W)
    
    # 4. 归一化边缘强度到 [0, 1]（Soft Dice 需要）
    pred_vessel_prob = pred_edges / (pred_edges.max() + 1e-8)
    gt_vessel_prob = gt_edges / (gt_edges.max() + 1e-8)
    
    # 5. 创建有效像素掩码（排除黑色配准边缘区域）
    threshold = 0.01
    # 检测黑色像素：所有通道都小于阈值
    ...
    # 应用掩码（保持梯度）
    pred_vessel_masked = pred_vessel_prob * valid_mask.float()
    gt_vessel_masked = gt_vessel_prob * valid_mask.float()
    
    # 6. 计算 Soft Dice Loss
    intersection = (pred_vessel_masked * gt_vessel_masked).sum(dim=[2, 3])  # (B,1)
    union = pred_vessel_masked.sum(dim=[2, 3]) + gt_vessel_masked.sum(dim=[2, 3])  # (B,1)
    dice = (2. * intersection + smooth) / (union + smooth)  # (B,1)
    
    # Dice Loss = 1 - Dice（越小越好）
    vessel_loss = 1 - dice.mean()
    
    return vessel_loss


def get_dynamic_loss_weights(global_step):
    """
    动态损失权重调度
    
    Phase 1 (step < 3000): 全局结构学习
        - mse=1.0, msssim=0.1, vessel=0.05
    
    Phase 2 (step >= 3000): 细节优化
        - mse=0.3, msssim=0.05, vessel=0.5
    
    返回: (mse_weight, msssim_weight, vessel_weight)
    """
    if global_step < 3000:
        return 1.0, 0.1, 0.05
    else:
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    """
    运行推理测试（每500步）- Dual ControlNet 版本 v8-1 (Scribble + Tile)
    ...
    # 2. 生成Scribble条件图（Vessel血管图或HED边缘图）
    if is_cffa:
        # CF-FA模式：使用Vessel血管提取
        from data_loader_cffa_v2 import extract_vessel_hessian
        image_type = 'cf' if mode == "cf2fa" else 'fa'
        cond_scribble_pil = extract_vessel_hessian(
            src_img_original,
        ...
        # CF-OCTA模式：使用HED边缘检测
        from data_loader_cfocta import get_hed_detector
        hed_detector = get_hed_detector()
        cond_scribble_pil = hed_detector(src_img_original)
    
    # Resize Scribble条件图
    cond_scribble_pil = cond_scribble_pil.resize((SIZE, SIZE))
    
    # 3. Tile ControlNet 使用原始彩色图（保留颜色和细节信息）
    cond_tile_pil = src_img_original.resize((SIZE, SIZE))
    
    # 4. 保存预处理结果
    idx = os.path.splitext(os.path.basename(src_path))[0]
    
    # 确定Scribble权重（CF-OCTA模式权重为0）
    scribble_scale = 0.0 if not is_cffa else args.scribble_scale
    ...
    if is_cffa:
        ...
        # CF-OCTA 模式
        cond_scribble_pil.save(os.path.join(infer_dir, f"{idx}_condition_scribble_hed.png"))
    ...
    print(f"  Scribble ControlNet 强度: {scribble_scale} {'(禁用)' if scribble_scale == 0 else ''}")
...
def main():
    ...
    # CF-OCTA模式：自动禁用Scribble ControlNet（权重设为0）
    if not is_cffa:
        if args.scribble_scale != 0:
            print(f"\n注意: CF-OCTA 模式自动禁用 Scribble ControlNet")
            print(f"  Scribble权重: {args.scribble_scale} → 0.0 (保持单路Tile行为)")
            args.scribble_scale = 0.0
    ...
    t_block = time.time()
    loss_accumulator = []
    msssim_loss_accumulator = []  # v8-1: 总是初始化
    vessel_loss_accumulator = []  # v8-1: 新增

    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v8-1】✨")
    print("="*70)
    print(f"  模型: Stable Diffusion 1.5 (512×512)")
    print(f"  ControlNet 架构: 双路 (Scribble + Tile)")
    print(f"    - Scribble: {'Vessel血管引导' if is_cffa else 'HED边缘引导 (权重=0)'} (强度 {args.scribble_scale})")
    ...
    print(f"\n  【v8-1 新特性】")
    print(f"  ├─ 损失函数: MSE + MS-SSIM + Vessel Loss (Sobel+Dice)")
    print(f"  ├─ 动态权重调度:")
    print(f"  │   Phase 1 (0-3000): mse=1.0, msssim=0.1, vessel=0.05")
    print(f"  │   Phase 2 (3000+): mse=0.3, msssim=0.05, vessel=0.5")
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            if global_step == 0:
                ...
                print(f"  1. {cond_filename}_scribble_input.png - Scribble ControlNet 输入（{'Vessel血管' if is_cffa else 'HED边缘'}）")
            ...
            # ============ v8-1: 获取动态损失权重 ============
            mse_weight, msssim_weight, vessel_weight = get_dynamic_loss_weights(global_step)
            
            # 计算 MSE 损失（噪声预测）
            loss_mse = mse(noise_pred, noise)

            # --- MS-SSIM 感知损失 + Vessel 结构损失 ---
            # 需要 VAE 解码到像素空间
            with torch.no_grad():
            ...
            # 4. 【v8-1 新增】血管结构损失 (Sobel + Soft Dice)
            loss_vessel = compute_vessel_loss(pred_imgs, tgt_imgs)
            
            # 5. 组合损失（使用动态权重）
            loss = (mse_weight * loss_mse + 
                    msssim_weight * loss_msssim + 
                    vessel_weight * loss_vessel)
            ...
            vessel_loss_accumulator.append(loss_vessel.item())  # v8-1 新增
            global_step += 1
            ...
            if global_step % 100 == 0:
                ...
                # ============ v8-1: 获取当前动态权重 ============
                mse_w, msssim_w, vessel_w = get_dynamic_loss_weights(global_step)
                
                # 构建日志消息
                msg_parts = [
                    f"[SD15-v8-1] step {global_step}/{max_steps}",
                    f"lr:{current_lr:.2e}",
                    f"mse:{avg_mse:.4f}(w={mse_w:.2f})",
                    f"vessel:{avg_vessel:.4f}(w={vessel_w:.2f})"
                ]
                
                if args.msssim_lambda > 0:
                    msg_parts.append(f"msssim:{avg_msssim:.4f}(w={msssim_w:.2f})")
            ...
            # ============ v8-1: 保存 checkpoint 并运行推理测试（每500步）============
    ...
    print(f"  模型版本: SD 1.5 + Dual ControlNet v8-1 (Scribble + Tile, FP32)")
    print(f"  ControlNet 强度: Scribble={args.scribble_scale}, Tile={args.tile_scale}")
    print(f"  损失函数: MSE + MS-SSIM + Vessel Loss (动态权重)")
