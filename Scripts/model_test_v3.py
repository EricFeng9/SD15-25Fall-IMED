...
- 完全避免 Frangi 滤波产生的边界伪影问题
- 保留所有血管细节，让 ControlNet 自己学习提取特征
- 根据模式自动决定是否取反：
  * 需要取反：cf2fa, cf2octa（让暗血管变亮）
  * 不取反：cf2oct（保持原样）
- 应用 CLAHE 对比度增强，提升血管可见度

【v10 更新】Single Source of Truth（单一数据源）
- 血管提取函数和参数从 data_loader_all_v11.py 统一导入
- 使用 extract_green_channel_for_scribble（与训练脚本 v11 完全一致）
- 确保推理与训练使用完全相同的预处理逻辑
...
2. 【正向】根据模态，resize_with_padding到训练集CF图尺寸（记录padding信息）
   - CF-FA: 720×576, CF-OCT: 1016×675, CF-OCTA: 400×400
3. Resize到512×512
4. 【v11】提取绿色通道作为Scribble输入（与训练脚本v11完全一致）
   - 根据模式自动决定是否取反（cf2fa/cf2octa需要取反）
   - 应用CLAHE对比度增强
5. 模型推理生成 512×512 结果
...
【v11特性】Scribble输入改用绿色通道，与训练脚本v11完全对齐
【v10特性】预处理逻辑与训练完全一致（Single Source of Truth）
...
# 【v11 改进】从统一数据加载器导入绿色通道提取函数（与训练脚本v11对齐）
from data_loader_all_v11 import (
    extract_green_channel_for_scribble,  # v11: 绿色通道提取（替代Frangi滤波）
    SIZE                                  # 模型输入尺寸 512×512
)
...
# ============ 其他配置 ============
# SIZE = 512  # 已从 data_loader_all_v11 导入
...
with open(log_path, "w") as f:
    f.write("="*70 + "\n")
    f.write("模型测试脚本 v11 - 批量推理（与训练脚本v11对齐）\n")
    f.write("="*70 + "\n")
    ...
    f.write("【v11更新】✨ 🎯 Scribble输入改用绿色通道（与训练脚本v11完全对齐）\n")
    f.write("  - 不再使用Frangi滤波，完全避免边界伪影问题\n")
    f.write("  - 根据模式自动决定是否取反：\n")
    f.write("    * cf2fa, cf2octa: 需要取反（暗血管→亮血管）\n")
    f.write("    * cf2oct: 不取反（保持原样）\n")
    f.write("  - 应用CLAHE对比度增强，提升血管可见度\n")
    f.write("  - 保留所有血管细节，让ControlNet自己学习提取特征\n")
    ...
    f.write("【架构】双路 ControlNet: Scribble(绿色通道) + Tile\n")
    f.write("="*70 + "\n\n")
...
print(f"  【v11更新】✨ 🎯 Scribble输入: 绿色通道（不再用Frangi滤波，与训练脚本v11对齐）")
...
for i, img_path in enumerate(image_files):
    try:
        ...
        for mode in modes_to_run:
            ...
            # Step 3: 【v11 改进】使用绿色通道提取（与训练脚本v11完全一致）
            # 根据模式自动决定是否取反，应用CLAHE增强
            cond_scribble = extract_green_channel_for_scribble(
                cf_512_pil,
                mode=mode,
                apply_clahe=True  # 应用CLAHE对比度增强
            )
            
            # cond_tile直接用512×512的原图
            ...
            # 注意：cond_scribble 和 cond_tile 都是 PIL Image 对象
            with torch.no_grad():
                img = pipe(
                    prompt=args.prompt,
                    negative_prompt=args.negative_prompt if args.negative_prompt else None,
                    image=[cond_scribble, cond_tile],  # [Scribble绿色通道, Tile原图]
        ...
        with open(log_path, "a") as f:
            ...
            f.write(f"  蒙版参数: threshold={MASK_THRESHOLD}, smooth={MASK_SMOOTH}, kernel_size={MASK_KERNEL_SIZE}\n")
            f.write(f"  【v11更新】✨ Scribble输入改用绿色通道（与训练脚本v11对齐）:\n")
            f.write(f"    - 不再使用Frangi滤波，完全避免边界伪影问题\n")
            if "cf2fa" in modes_to_run:
                f.write(f"    - CF-FA模式: 绿色通道 + 取反 + CLAHE增强\n")
            if "cf2oct" in modes_to_run:
                f.write(f"    - CF-OCT模式: 绿色通道（不取反）+ CLAHE增强\n")
            if "cf2octa" in modes_to_run:
                f.write(f"    - CF-OCTA模式: 绿色通道 + 取反 + CLAHE增强\n")
            f.write(f"  【v5反向resize】处理流程:\n")
            f.write(f"    正向: 输入CF图({original_width}×{original_height}) → resize_with_padding到训练集尺寸 → resize到512×512 → 提取绿色通道 → 推理\n")
...
print(f"  架构: 双路 ControlNet (Scribble + Tile) v11")
print(f"  推理模式: {', '.join(modes_to_run)}")
...
print(f"  【v11更新】✨ Scribble输入: 绿色通道（不再用Frangi，与训练脚本v11完全对齐）")
print(f"  【v5更新】反向resize: 512×512输出 → 训练集尺寸 → 裁剪padding → 原图尺寸（结构完全对齐）")
print(f"  【v4更新】训练集尺寸对齐: CF-FA={TRAIN_SIZE_CF2FA}, CF-OCT={TRAIN_SIZE_CF2OCT}, CF-OCTA={TRAIN_SIZE_CF2OCTA}")
print(f"  【v2更新】智能黑边蒙版: threshold={MASK_THRESHOLD}, smooth={MASK_SMOOTH}, kernel_size={MASK_KERNEL_SIZE}")
print(f"{'='*70}\n")
...
with open(log_path, "a") as f:
    ...
    f.write(f"【v11更新】✨ Scribble输入改用绿色通道（与训练脚本v11完全对齐）\n")
    f.write(f"  不再使用Frangi滤波，完全避免边界伪影问题\n")
    f.write(f"  根据模式自动取反: cf2fa/cf2octa需要取反, cf2oct不取反\n")
    f.write(f"  应用CLAHE对比度增强，保留所有血管细节\n")
