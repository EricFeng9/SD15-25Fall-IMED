...
# ============ 参数解析 ============
parser = argparse.ArgumentParser(description="SD 1.5 + Dual ControlNet 推理脚本 v9-3 (支持 CF-OCTA、CF-FA 和 CF_OCT，智能黑边蒙版)")
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + Dual ControlNet 推理参数 (v9-3 双路架构 - 所有模式)\n")
    f.write("="*70 + "\n")
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        # ControlNet 预处理 - 支持 CF-FA、CF-OCTA 和 CF_OCT
        # 【v9-2 修复】根据数据集类型选择不同的预处理策略
        
        if is_cffa:
            # CF-FA 模式：与训练一致 - 先从原图提取血管，再 resize
            ...
            # CF_OCT 模式：【v9-2 新配准方案】先 resize_with_padding，再提取血管
            ...
            # CF-OCTA 模式：与训练一致 - 先从原图提取血管，再 resize
...
print(f"  架构: 双路 ControlNet (Scribble + Tile) - v9-3")
print(f"  数据集: {'CF-FA' if is_cffa else ('CF_OCT' if is_cfoct else 'CF-OCTA')}")
