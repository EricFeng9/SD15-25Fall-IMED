# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v8-3

【基于】train_controlnet_sd15_v8-2.py
...
- **【v8-3新增】Frangi血管滤波 + L1距离损失**
- FP32 训练

【v8-3 更新】✨
1. **血管结构损失 (Vessel Loss) - Frangi版本**
   - 使用 Frangi 血管滤波（专为管状结构设计）+ L1 距离（完全可微）
   - 参数与 vessle_detector.py 一致：sigmas=range(1,50), gamma=15
   - CF图像处理：提取绿色通道 + 取反（暗血管→亮血管）
   - 固定权重（可通过 --vessel_lambda 调节，默认0.05）

2. **多损失函数训练**
   - MSE Loss (噪声预测) - 权重固定 1.0
   - MS-SSIM Loss (感知质量) - 权重固定 0.1 (可调)
   - Vessel Loss (Frangi + L1) - 权重固定 0.05 (可调)
   - 所有权重从头到尾保持不变，避免后期变形
...
【使用方法】
# CF-FA 训练（双路Scribble+Tile模式，带Frangi血管损失）
python train_controlnet_sd15_v8-2.py --mode cf2fa --name sd15_v8-3_cffa --max_steps 8000 --scribble_scale 0.8 --vessel_lambda 0.05
...
参数调节:
  ...
  --vessel_lambda: Vessel Loss 权重 (默认 0.05，固定不变)
'''
...
def compute_vessel_loss_frangi(pred_imgs, gt_imgs, mode='cf2fa', sigmas=range(1, 16), 
                                alpha=0.5, beta=0.5, gamma=0.015, debug_dir=None):
    """
    血管结构损失 - 使用 Frangi 滤波 + L1 距离
    
    【与 data_loader_cffa_v2.py 的 extract_vessel_hessian 完全一致的处理】
    - CF图: 绿色通道 + 取反（血管是暗色）
    - FA图: 绿色通道 + 不取反（血管是亮色）
    ...
    参数:
        ...
        mode: 训练模式 ('cf2fa', 'fa2cf', 等)
        sigmas: Frangi 多尺度参数（默认 range(1, 16)，与 data_loader 一致）
        alpha: Frangi 板状结构敏感度（默认 0.5）
        beta: Frangi 球状结构敏感度（默认 0.5）
        gamma: Frangi 噪声抑制阈值（默认 0.015，与 data_loader 一致）
    ...
    # 2. 根据目标图像类型提取灰度图（与 extract_vessel_hessian 逻辑一致）
    # cf2fa / cf2octa: 目标是 FA/OCTA 图（血管是亮色，不反转）
    # fa2cf / octa2cf: 目标是 CF 图（血管是暗色，需反转）
    is_cf_target = mode in ['fa2cf', 'octa2cf']
    
    # 提取绿色通道（与 extract_vessel_hessian 一致）
    pred_green = pred_01[:, 1:2, :, :]  # (B, 1, H, W)
    gt_green = gt_01[:, 1:2, :, :]
    
    if is_cf_target:
        # 目标是 CF 图：绿色通道 + 取反（血管是暗色）
        pred_gray = 1.0 - pred_green
        gt_gray = 1.0 - gt_green
    else:
        # 目标是 FA/OCTA 图：绿色通道，不取反（血管是亮色）
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    """
    运行推理测试（每500步）- Dual ControlNet 版本 v8-3 (Scribble + Tile)
...
def main():
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v8-3】✨")
    print("="*70)
    ...
    print(f"  ├─ Frangi 滤波参数（与 data_loader_cffa_v2.py 一致）:")
    print(f"  │   Sigmas: range(1, 16) - 多尺度血管检测")
    print(f"  │   Gamma: 0.015 - 噪声抑制阈值")
    print(f"  │   图像处理: CF图(绿色通道+反转) / FA图(绿色通道)")
    print(f"  ├─ 固定损失权重:")
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            loss_vessel = compute_vessel_loss_frangi(
                pred_imgs, tgt_imgs, 
                mode=args.mode,
                sigmas=range(1, 16),  # 与 data_loader_cffa_v2.py 一致
                alpha=0.5, 
                beta=0.5, 
                gamma=0.015,  # 与 data_loader_cffa_v2.py 一致
            ...
            vessel_loss_accumulator.append(loss_vessel.item())  # v8-3 新增
            global_step += 1
            ...
            if global_step % 100 == 0:
                ...
                # 构建日志消息（v8-3: 显示固定权重）
                msg_parts = [
                    f"[SD15-v8-3] step {global_step}/{max_steps}",
                    f"lr:{current_lr:.2e}",
                    f"mse:{avg_mse:.4f}",
    ...
    print(f"  模型版本: SD 1.5 + Dual ControlNet v8-3 (Scribble + Tile, FP32)")
    ...
    print(f"  Frangi 参数: sigmas=range(1,50), gamma=15")
