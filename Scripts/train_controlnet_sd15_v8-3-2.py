# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v8-3-2
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    ...
    if is_cffa:
        ...
        # CF-OCTA 模式（v8-3-2）：CF训练集已改为彩色原图，直接使用原图
        # cf2octa: 直接使用 CF 彩色原图
...
def main():
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v8-3-2】✨")
    print("="*70)
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            if global_step % 100 == 0:
                ...
                # 构建日志消息
                msg_parts = [
                    f"[SD15-v8-3-2] step {global_step}/{max_steps}",
                    f"lr:{current_lr:.2e}",
                    f"mse:{avg_mse:.4f}",
                ]
    ...
    print(f"  模型版本: SD 1.5 + Dual ControlNet v8-3-2 (Scribble + Tile, FP32)")
    print(f"  数据集类型: {'CF-FA' if is_cffa else 'CF-OCTA'}")
    if not is_cffa:
