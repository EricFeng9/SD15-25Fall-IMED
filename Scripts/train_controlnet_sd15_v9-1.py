# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v9-1
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    ...
    if is_cffa:
        ...
        gamma_value = GAMMA_CFOCT_CF if image_type == 'cf' else GAMMA_CFOCT_OCT
        cond_scribble_pil = extract_vessel_hessian(
            src_img_original,
            image_type=image_type,
            gamma=gamma_value,
        ...
        cond_scribble_pil = extract_vessel_frangi_cfocta(
            ...
            image_type=image_type,
            sigmas=FRANGI_SIGMAS,
            gamma_cf=GAMMA_CFOCTA_CF,    # CF图的gamma值
            gamma_octa=GAMMA_CFOCTA_OCTA   # OCTA图的gamma值
        )
        # Resize Scribble条件图
        cond_scribble_pil = cond_scribble_pil.resize((SIZE, SIZE))
...
def main():
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v9-1】✨")
    print("="*70)
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            if global_step % 100 == 0:
                ...
                # 构建日志消息
                msg_parts = [
                    f"[SD15-v9-1] step {global_step}/{max_steps}",
                    f"lr:{current_lr:.2e}",
                    f"mse:{avg_mse:.4f}",
                ]
    ...
    print(f"  模型版本: SD 1.5 + Dual ControlNet v9-1 (Scribble + Tile, FP32)")
    print(f"  数据集类型: {'CF-FA' if is_cffa else ('CF_OCT' if is_cfoct else 'CF-OCTA')}")
    print(f"  v9-1 更新: ")
    print(f"    1. MSE Loss 在噪声空间应用蒙版消除黑边影响")
    print(f"    2. 保持扩散模型标准训练范式（学习去噪，不是重建）")
    print(f"    3. MS-SSIM和Vessel Loss在像素空间应用蒙版")
    print(f"    4. 既消除黑边影响，又避免权重失衡和梯度问题")
