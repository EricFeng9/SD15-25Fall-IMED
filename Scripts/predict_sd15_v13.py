"""
SD 1.5 ControlNet 推理脚本 - v13

【v13 更新】✨ CF_OCT 配准逻辑统一化
- CF_OCT 配准使用统一数据加载器接口（data_loader_all_v13）
- 确保推理和训练使用完全相同的配准逻辑（Single Source of Truth）
- 不再直接调用底层配准函数，避免逻辑分散

【v11 更新】✨ Scribble输入改用绿色通道
- Scribble ControlNet输入从Frangi滤波改为绿色通道（+可选取反+CLAHE）
- 完全避免Frangi滤波产生的边界伪影问题
- 根据模式自动决定是否取反：
  * 需要取反：cf2fa, cf2octa, oct2cf（让暗血管变亮）
  * 不取反：cf2oct, fa2cf, octa2cf（保持原样）
- 与训练脚本v11保持完全一致
- Vessel Loss仍使用Frangi滤波（仅用于训练监督）

【v13 更新】CF_OCT 配准逻辑统一化
- CF_OCT 配准使用统一数据加载器接口（data_loader_all_v13._apply_registration_cfoct）
- 统一配准到 CF 域（1016×675），然后直接 resize 到 512×512
- 使用透视变换（perspective transformation）提高配准精度
- 确保推理和训练使用完全相同的配准逻辑（Single Source of Truth）
- 不再直接调用底层 registration_cf_oct 函数，避免逻辑分散

【v10-2 更新】统一配置参数 + 最佳模型支持
- 支持 --step best 加载最佳模型检查点（best_checkpoint/）
- FOV/掩码参数统一化：使用 get_image_params() 从 data_loader_all 获取
- 确保推理和训练使用完全相同的参数配置（Single Source of Truth）
- 删除硬编码的蒙版参数，所有参数根据 mode 自动从配置获取
- 适配训练脚本 v10-2 的早停机制和固定学习率特性
...
【使用说明】
基础用法（默认使用最佳模型）：
  # CF-OCTA 推理（双路 Scribble+Tile ControlNet，默认加载 best_checkpoint）
  python predict_sd15_v10-2.py --mode cf2octa --name sd15_v10-2_cfocta
  
  # CF-FA 推理（双路 Scribble+Tile，默认加载 best_checkpoint）
  python predict_sd15_v10-2.py --mode cf2fa --name sd15_v10-2_cffa
  
  # 使用特定步数的模型（如需要）
  python predict_sd15_v10-2.py --mode cf2fa --name sd15_v10-2_cffa --step 6000
...
可选参数：
  --step          使用特定训练步数的权重（默认：best，加载最佳模型）
                  例如：--step 6000 会加载 step_6000 目录的权重
                  特殊值：--step best 会加载 best_checkpoint 目录（v10-2 默认）
                  设置为 None 或空字符串可加载根目录的最终权重
...
【示例命令】
1. 最简单用法（默认使用最佳模型 best_checkpoint，自动评估）：
...
2. 使用特定步数的权重（覆盖默认的 best）：
   python predict_sd15_v10-2.py --mode cf2fa --name sd15_v10-2_cffa --step 8000

3. 使用自定义保存目录：
   python predict_sd15_v10-2.py --mode cf2fa --name sd15_v10-2_cffa --savedir test_results
...
7. 反向任务（FA→CF / OCTA→CF）：
   python predict_sd15_v10-2.py --mode fa2cf --name sd15_v10-2_cffa
   python predict_sd15_v10-2.py --mode octa2cf --name sd15_v10-2_cfocta

【输出】
- 生成图像保存在：out_preds_sd15_dual/{mode}/{name}/[best|step_{N}|savedir]/
  * 默认保存到 best/ 子目录（使用最佳模型）
...
from data_loader_all_v13 import (
    preprocess_for_vessel_extraction,  # v11: 统一预处理接口（Scribble改用绿色通道）
    GAMMA_CFFA, GAMMA_CFOCTA_CF, GAMMA_CFOCTA_OCTA,  # v10: Frangi参数（仅用于Vessel Loss）
    GAMMA_CFOCT_CF, GAMMA_CFOCT_OCT, FRANGI_SIGMAS,
    get_image_params,  # v10-2: 统一图像处理参数配置
    UnifiedDataset  # v13: 统一配准接口
)
...
# ============ 参数解析 ============
# 注意：FOV/掩码参数不再硬编码，使用 get_image_params() 从统一配置获取
parser = argparse.ArgumentParser(description="SD 1.5 + Dual ControlNet 推理脚本 v10-2 (统一配置参数 + 最佳模型支持，支持 CF-OCTA、CF-FA 和 CF_OCT)")
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + Dual ControlNet 推理参数 (v10-2 统一配置 - 所有模式)\n")
    f.write("="*70 + "\n")
    ...
    f.write("【v13更新】CF_OCT配准逻辑统一化:\n")
    f.write("  - CF_OCT配准使用统一数据加载器接口（data_loader_all_v13._apply_registration_cfoct）\n")
    f.write("  - 统一配准到CF域（1016×675），然后直接resize到512×512\n")
    f.write("  - 使用透视变换（perspective transformation）提高配准精度\n")
    f.write("【v10-2更新】统一配置参数 + 最佳模型支持:\n")
    f.write("  - 支持 --step best 加载最佳模型检查点（best_checkpoint/）\n")
    f.write("  - FOV/掩码参数统一化: 使用 get_image_params() 从 data_loader_all 获取\n")
    f.write("  - 确保推理和训练使用完全相同的参数配置（Single Source of Truth）\n")
    ...
    f.write(f"  蒙版参数: 所有参数根据 mode 自动从 get_image_params() 获取\n")
...
print(f"  【v13更新】CF_OCT配准逻辑统一化:")
print(f"    - CF_OCT配准使用统一数据加载器接口（data_loader_all_v13._apply_registration_cfoct）")
print(f"    - 统一配准到CF域（1016×675），然后直接resize到512×512")
print(f"    - 使用透视变换（perspective transformation）提高配准精度")
print(f"  【v10-2更新】统一配置参数 + 最佳模型支持:")
print(f"    - 支持 --step best 加载最佳模型检查点（best_checkpoint/）")
print(f"    - FOV/掩码参数统一化: 使用 get_image_params() 从 data_loader_all 获取")
print(f"    - 确保推理和训练使用完全相同的参数配置（Single Source of Truth）")
...
print(f"    蒙版参数: 所有参数根据 mode 自动从 get_image_params() 获取")
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        # 【v10-2】生成黑边蒙版（基于原始图像，使用统一配置参数）
        # 从 get_image_params() 获取蒙版参数（根据 mode 自动选择正确配置）
        cond_params = get_image_params(args.mode, param_type='condition')
        
        src_img_np = np.array(src_img_original)
        mask_original = mask_gen(
            src_img_np, 
            threshold=cond_params['fov_threshold'], 
            smooth=True,  # 固定为 True（平滑边缘）
            kernel_size=5  # 固定为 5（标准核大小）
        ...
        if is_cfoct:
            ...
            # 2. 【v9-3】应用智能黑边蒙版到预测图
            # 【v13更新】直接 resize mask 到 512×512（与新配准方案一致）
        ...
        try:
            ...
            if is_cffa:
                ...
                # CF_OCT 模式：【v13 新方案】使用统一数据加载器配准接口
                # 确保推理和训练使用完全相同的配准逻辑（Single Source of Truth）
                if need_register and isinstance(affine_path, tuple):
                    cond_pts_path, tgt_pts_path = affine_path
                    
                    # 根据 mode 确定 cf_pts_path 和 oct_pts_path
                    if args.mode == "cf2oct":
                        # CF→OCT: src=CF, target=OCT
                        cf_pts_path, oct_pts_path = cond_pts_path, tgt_pts_path
                    else:  # oct2cf
                        # OCT→CF: src=OCT, target=CF
                        oct_pts_path, cf_pts_path = cond_pts_path, tgt_pts_path
                    
                    if cf_pts_path and oct_pts_path and os.path.exists(cf_pts_path) and os.path.exists(oct_pts_path):
                        # 【v13 新方案】使用 data_loader 统一配准接口
                        # 不再直接调用底层函数，避免逻辑分散
                        temp_ds = UnifiedDataset.__new__(UnifiedDataset)
                        temp_ds.mode = args.mode
                        
                        _, target_img_registered = temp_ds._apply_registration_cfoct(
                            src_img_pil=src_img_original,
                            target_img_pil=target_img_preprocessed,
                            cf_pts_path=cf_pts_path,
                            oct_pts_path=oct_pts_path
                        )
                    else:
                        # 没有关键点时，直接 resize 到 512×512
                        target_img_registered = target_img_preprocessed.resize((SIZE, SIZE))
                else:
                    # 不需要配准时，直接 resize 到 512×512
                    target_img_registered = target_img_preprocessed.resize((SIZE, SIZE))
            else:
...
print(f"  架构: 双路 ControlNet (Scribble + Tile) - v10-2")
print(f"  数据集: {'CF-FA' if is_cffa else ('CF_OCT' if is_cfoct else 'CF-OCTA')}")
...
print(f"  【v10-2更新】统一配置参数 + 最佳模型支持:")
print(f"    - 支持 --step best 加载最佳模型检查点（best_checkpoint/）")
print(f"    - FOV/掩码参数统一化: 使用 get_image_params() 从统一配置获取")
print(f"    - 确保推理和训练使用完全相同的参数配置（Single Source of Truth）")
print(f"  【v10更新】统一数据加载器 + FOV圆形掩码:")
print(f"    - 使用统一数据加载器（data_loader_all.py）")
print(f"    - 在Frangi血管滤波后应用FOV圆形掩码，移除边界伪影")
...
if len(valid_metrics) > 0:
    ...
    with open(log_path, "a") as f:
        ...
        f.write("    【v10-2更新】统一配置参数 + 最佳模型支持:\n")
        f.write("      - 支持 --step best 加载最佳模型检查点（best_checkpoint/）\n")
        f.write("      - FOV/掩码参数统一化: 使用 get_image_params() 从统一配置获取\n")
        f.write("      - 确保推理和训练使用完全相同的参数配置（Single Source of Truth）\n")
        f.write("    【v10更新】统一数据加载器 + FOV圆形掩码:\n")
        f.write("      - 使用统一数据加载器（data_loader_all.py）\n")
        f.write("      - 在Frangi血管滤波后应用FOV圆形掩码，移除边界伪影\n")
