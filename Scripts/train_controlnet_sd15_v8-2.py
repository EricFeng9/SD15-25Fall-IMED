# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v8-2
...
1. **血管结构损失 (Vessel Loss)**
   ...
   - 直接约束血管结构，防止细节过度平滑
   - 固定权重（可通过 --vessel_lambda 调节）

2. **多损失函数训练**
   - MSE Loss (噪声预测) - 权重固定 1.0
   - MS-SSIM Loss (感知质量) - 权重固定 0.1 (可调)
   - Vessel Loss (血管结构) - 权重固定 0.5 (可调)
   - 所有权重从头到尾保持不变，避免后期变形
...
python train_controlnet_sd15_v8.py --mode cf2fa --name sd15_v8-2_cffa --max_steps 8000 --scribble_scale 0.8

参数调节:
  --scribble_scale: Scribble ControlNet 强度 (默认 0.8，CF-OCTA自动设为0)
  --tile_scale: Tile ControlNet 强度 (默认 1.0)
  --msssim_lambda: MS-SSIM 损失权重 (默认 0.1，固定不变)
  --vessel_lambda: Vessel Loss 权重 (默认 0.5，固定不变)
'''
...
def main():
    ...
    parser.add_argument("--vessel_lambda", type=float, default=0.5,
                        help="Vessel Loss 血管结构损失的权重 (默认0.5)")
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v8-2】✨")
    print("="*70)
    ...
    print(f"  ├─ 固定损失权重:")
    print(f"  │   MSE (噪声预测): 1.0 (固定)")
    print(f"  │   MS-SSIM (感知质量): {args.msssim_lambda} (固定)")
    print(f"  │   Vessel (血管结构): {args.vessel_lambda} (固定)")
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            # Vessel 权重由 --vessel_lambda 控制（默认 0.5）
            loss = (1.0 * loss_mse + 
            ...
            if global_step % 100 == 0:
                ...
                msg_parts = [
                    ...
                    f"lr:{current_lr:.2e}",
                    f"mse:{avg_mse:.4f}",
                    f"vessel:{avg_vessel:.4f}(λ={args.vessel_lambda})"
                ]
    ...
    print(f"  模型版本: SD 1.5 + Dual ControlNet v8-2 (Scribble + Tile, FP32)")
    print(f"  ControlNet 强度: Scribble={args.scribble_scale}, Tile={args.tile_scale}")
    print(f"  损失函数: MSE + MS-SSIM(λ={args.msssim_lambda}) + Vessel(λ={args.vessel_lambda})")
