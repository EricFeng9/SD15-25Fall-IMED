# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v12 【方案A: GT-only监督】

【基于】train_controlnet_sd15_v11-2.py
【模型】Stable Diffusion 1.5 (512×512) + 双路 ControlNet (Scribble + Tile)
【数据集】支持三种数据集
  - CF-OCTA: 配准数据集 (绿色通道 + Tile)
  - CF-FA: CFFA 数据集 (绿色通道 + Tile)
  - CF-OCT: CF_OCT 数据集 (绿色通道 + Tile)

【v12 更新】✨ 🎯 【方案A: GT-only监督】
1. **蒙版策略改为GT-only（只在GT有效区域监督）**
   - 像素空间损失（MS-SSIM、梯度）：只看GT黑边，不看pred黑边
   - Vessel Loss：只看GT黑边和GT的FOV掩码
   - 噪声空间MSE：已是GT-only（保持不变）
   - 原理：只在有GT的地方精确学习，在没GT的地方完全不管
   
2. **推理CFG可调（默认5.5，支持命令行调整）**
   - 降低CFG强度，允许模型在GT缺失区域更自然外推
   - 依靠ControlNet全图条件引导在推理时自动补全
   
3. **预期效果**
   - 训练：GT有效区域精确监督，GT缺失区域零监督
   - 推理：依靠Scribble条件覆盖全图，自然延伸到GT缺失区域

【v11 更新】✨ 🎯
1. **Scribble ControlNet 输入改用绿色通道（不再使用Frangi滤波）**
   - 完全避免Frangi滤波产生的边界伪影问题
   - 保留所有血管细节，让ControlNet自己学习提取特征
   - 根据模式自动决定是否取反：
     * 需要取反：cf2fa, cf2octa, oct2cf（让暗血管变亮）
     * 不取反：cf2oct, fa2cf, octa2cf（保持原样）
   - 应用CLAHE对比度增强，提升血管可见度
   
2. **Vessel Loss 保持使用 Frangi 滤波 + Dice Loss**
   - Scribble输入简化，但Vessel Loss仍用Frangi提供显式监督
   - 确保模型学习正确的血管结构
   - 继续使用FOV掩码和侵蚀处理（仅用于Vessel Loss计算）

【v10-3 更新】✨ 🎯
1. **Vessel Loss 改用 Dice Loss 计算（针对稀疏血管图优化）**
   - 从 MS-SSIM 改为 Dice Loss（专为稀疏二值结构设计）
   - 解决背景主导效应（90%黑色背景淹没10%血管信号）
   - 不受位置偏移影响（关注重叠度而非逐像素对齐）
   - Dice系数：2×|A∩B| / (|A|+|B|)，专注血管区域重叠
   - 训练更稳定，Loss信号更明确
   - 移除不必要的3通道扩展

【v10-3 更新】✨ 🎯（已废弃，改用Dice）
1. **Vessel Loss 改用 MS-SSIM 计算**
   - 发现问题：稀疏血管图不适合SSIM（背景主导效应）
   - 已在v10-3中改为Dice Loss
...
- 双路 ControlNet 架构（血管结构 + 原图细节）
  * ControlNet-Scribble: 血管结构引导（【v11】使用绿色通道+可选取反+CLAHE）
  * ControlNet-Tile: 原图细节保留（纹理、强度、颜色）
- SD 1.5 原生支持 512×512 分辨率
- **【v11更新】Scribble输入改用绿色通道，避免Frangi边界伪影**
- **【v8-3-2更新】CF训练集改用彩色原图，Tile输入直接使用原图**
- Vessel Loss继续使用Frangi滤波提供显式监督
- FP32 训练
...
2. **血管结构损失 (Vessel Loss) - 所有模式启用**
   - 【v10-3】使用 Frangi 血管滤波 + Dice Loss（专为稀疏血管图设计）
...
3. **多损失函数训练**
   ...
   - Vessel Loss (Frangi + Dice) - 权重固定 0.05 (所有模式) 【v10-3改进】
   - 所有权重从头到尾保持不变，避免后期变形
...
def rgb_to_luma_y(image_01, weights=(0.2126, 0.7152, 0.0722)):
    """
    将RGB图像转换为亮度Y通道（范围 [0,1]），默认使用BT.709系数。
    
    Args:
        image_01: (B, 3, H, W) tensor，范围 [0, 1]
        weights: (wR, wG, wB) 亮度权重
    Returns:
        y: (B, 1, H, W) tensor
    """
    w = torch.tensor(weights, device=image_01.device, dtype=image_01.dtype).view(1, 3, 1, 1)
    y = (image_01 * w).sum(dim=1, keepdim=True)
    return y


def compute_gradient_match_loss(pred_imgs_01, gt_imgs_01, mask=None, reduction="l1"):
    """
    梯度匹配损失：在亮度Y通道上约束预测图与目标图的梯度一致。
    ...
    Returns:
        标量损失值
    """
    # 使用亮度Y通道（BT.709）计算梯度，避免仅约束G通道导致的色偏/条纹逃逸
    pred_gray = rgb_to_luma_y(pred_imgs_01)  # (B,1,H,W)
    gt_gray = rgb_to_luma_y(gt_imgs_01)      # (B,1,H,W)
...
def compute_vessel_loss_frangi(pred_imgs, gt_imgs, mode='cf2fa', sigmas=FRANGI_SIGMAS, 
    ...
    【v12 更新】🎯 【方案A: GT-only监督】
    - **【掩码策略改为GT-only】** 只看GT的黑边和GT的FOV掩码
    - valid_mask = ~black_mask_gt（不再看pred的黑边）
    - combined_mask = valid_mask * gt_fov_mask_batch（只用GT的FOV）
    - 原理：只在GT有效区域监督，GT缺失区域零梯度
    
    【v10-3 更新】🎯
    - 改用 Dice Loss 计算血管结构相似性（替代 MS-SSIM）
    - 解决稀疏血管图的背景主导效应（90%黑色背景淹没10%血管信号）
    - Dice系数专注于血管区域的重叠度：2×|A∩B| / (|A|+|B|)
    - 不受1-2像素位置偏移影响（关注整体重叠而非逐像素对齐）
    - 训练更稳定，Loss信号更明确，收敛更快
    ...
    # 4. 【v12 方案A】计算 Dice Loss（专为稀疏血管图设计）
    # 【v12改动】只用GT的掩码（GT-only监督策略）
    # combined_mask = GT的有效区域 ∩ GT的FOV掩码
    combined_mask = valid_mask * gt_fov_mask_batch  # (B, 1, H, W) 【v12简化】
    ...
    if debug_dir is not None:
        ...
        print(f"  - vessel_loss_pred_fov_mask.png: 预测图FOV侵蚀掩码（erode={erode_pixels}px）")
        print(f"  - vessel_loss_gt_fov_mask.png: GT图FOV侵蚀掩码（erode={erode_pixels}px）")
        print(f"  - vessel_loss_combined_mask.png: 组合掩码（pred∩gt∩valid的交集）\n")
...
def compute_total_loss(noise_pred, noise, noisy_latents, latents, timesteps, 
    ...
    # 【v12 方案A】创建像素空间蒙版（GT-only，只看GT黑边）
    threshold = 0.01
    black_mask_tgt = torch.all(tgt_imgs_0_1 <= threshold, dim=1, keepdim=True)
    black_mask_pred = torch.all(pred_imgs_0_1 <= threshold, dim=1, keepdim=True)  # 保留用于调试
    
    # GT-only蒙版：只在GT有效区域监督
    gt_valid_mask_1ch = ~black_mask_tgt  # (B, 1, H, W) 【v12改动】
    gt_valid_mask_3ch = gt_valid_mask_1ch.expand(-1, 3, -1, -1).float()  # (B, 3, H, W)
    
    # 3. MS-SSIM 损失（像素空间应用GT-only蒙版）
    if args.msssim_lambda > 0:
        # 【v12 方案A】将GT黑边区域置零（只在GT有效区域监督）
        tgt_imgs_0_1_masked = tgt_imgs_0_1 * gt_valid_mask_3ch
        pred_imgs_0_1_masked = pred_imgs_0_1 * gt_valid_mask_3ch
    ...
    # 4. Vessel 损失（Frangi + Dice + FOV掩码 + 图像边界保护）【v12: GT-only蒙版】
    ...
    # 5. 梯度匹配损失（GT-only蒙版）
    grad_mask = gt_valid_mask_1ch  # 【v12改动】使用GT-only蒙版
    ...
    # 6. 【方案B】背景平滑约束（防止GT缺失区域出现高频条纹）
    if args.bg_smooth_lambda > 0:
        # 只约束GT黑边区域的梯度平方和（不约束具体颜色值）
        # 原理：减少高频噪声，但允许低频变化
        pred_y = rgb_to_luma_y(pred_imgs_0_1)  # 转换为亮度Y通道
        pred_grad_x, pred_grad_y = compute_image_gradients(pred_y)
        # 只在GT黑边区域（GT缺失区域）计算梯度平方和
        loss_bg_smooth = ((pred_grad_x**2 + pred_grad_y**2) * black_mask_tgt.float()).mean()
    else:
        loss_bg_smooth = torch.tensor(0.0, device=device)
    ...
    total_loss = (loss_mse + 
                  ...
                  args.bg_smooth_lambda * loss_bg_smooth)
    
    if return_components:
        return total_loss, loss_mse, loss_msssim, loss_vessel, loss_grad, loss_bg_smooth
    else:
        return total_loss
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    ...
    with torch.no_grad():
        output = pipe(
            ...
            guidance_scale=args.cfg_scale,  # 【v12改动】推理CFG可调（默认5.5）
    ...
    print(f"  推理CFG强度: {args.cfg_scale} 【v12新增】")
...
def main():
    ...
    parser.add_argument("--cfg_scale", type=float, default=5.5,
                        help="推理时的CFG强度 (默认5.5，推荐5.0-7.0，更低更自然外推)")
    
    # 【v12 方案B】背景平滑约束（可选，用于防止GT缺失区域条纹）
    parser.add_argument("--bg_smooth_lambda", type=float, default=0.0,
                        help="背景平滑约束权重 (默认0.0关闭，推荐0.01-0.02，只约束GT缺失区域的梯度，防止条纹)")
    ...
    bg_smooth_accumulator = []    # v12: 背景平滑损失（方案B）
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v12】✨ 🎯 【方案A: GT-only监督】")
    print("="*70)
    print(f"  模型: Stable Diffusion 1.5 (512×512)")
    print(f"  ControlNet 架构: 双路 (Scribble + Tile)")
    print(f"    - Scribble: 绿色通道引导 (强度 {args.scribble_scale}) 【v11更新】")
    ...
    print(f"\n  【v12 更新】✨ 🎯 【方案A: GT-only监督】")
    print(f"  ├─ 蒙版策略改为GT-only（只在GT有效区域监督）")
    print(f"  │   - MSE Loss (噪声空间): 已是GT-only（保持不变）")
    print(f"  │   - MS-SSIM Loss (像素空间): 改为GT-only")
    print(f"  │   - Vessel Loss (像素空间): 改为GT-only + GT的FOV掩码")
    print(f"  │   - Gradient Loss (像素空间): 改为GT-only")
    print(f"  ├─ 核心原理：只在有GT的地方精确学习，没GT的地方零监督")
    print(f"  │   GT缺失区域依靠ControlNet全图条件自然延伸")
    print(f"  ├─ 推理CFG可调: {args.cfg_scale} (降低外推保守性)")
    print(f"  │   推荐范围: 5.0-7.0，更低→更自然，更高→更保守")
    if args.bg_smooth_lambda > 0:
        print(f"  ├─ 【方案B启用】背景平滑约束: λ={args.bg_smooth_lambda}")
        print(f"  │   只约束GT缺失区域的梯度平方和，防止高频条纹")
        print(f"  │   不约束颜色值，允许低频变化")
    print(f"  └─ 预期效果:")
    print(f"      - 训练: GT有效区精确监督 + GT缺失区零监督")
    print(f"      - 推理: Scribble条件覆盖全图，自动补全缺失区域")
    
    print(f"\n  【v11 更新】✨ 🎯")
    print(f"  ├─ Scribble输入改用绿色通道（不再使用Frangi滤波）")
    print(f"  │   避免边界伪影问题，保留所有血管细节")
    print(f"  │   根据模式自动取反：{args.mode}")
    if args.mode in ['cf2fa', 'cf2octa', 'oct2cf']:
        print(f"  │   → 需要取反（暗血管变亮）")
    else:
        print(f"  │   → 不取反（保持原样）")
    print(f"  │   应用CLAHE对比度增强")
    print(f"  ├─ Vessel Loss保持使用Frangi滤波（显式血管监督）")
    print(f"\n  【v10-3 更新】✨ 🎯")
    print(f"  ├─ Vessel Loss 改用 Dice Loss 计算（针对稀疏血管图优化）")
    print(f"  │   从 MS-SSIM 改为 Dice Loss（专为稀疏二值结构设计）")
    print(f"  │   解决背景主导效应（90%黑色背景淹没10%血管信号）")
    print(f"  │   Dice系数专注血管区域重叠：2×|A∩B| / (|A|+|B|)")
    print(f"  │   不受1-2像素位置偏移影响（关注整体重叠度）")
    print(f"  │   训练更稳定，Loss信号更明确，收敛更快")
    print(f"\n  【v10-2 更新】✨")
    ...
    print(f"  ├─ 损失函数: MSE + MS-SSIM + Vessel Loss (Frangi+Dice) 【v10-3改进】")
    print(f"  ├─ Frangi 滤波参数:")
    print(f"  │   Sigmas: range(1, 16) - 多尺度血管检测")
    if is_cffa:
        print(f"  │   Gamma: 0.015 (CF-FA数据集，仅用于Vessel Loss)")
        print(f"  │   Scribble输入: 绿色通道 + {('取反' if args.mode == 'cf2fa' else '不取反')} + CLAHE")
        print(f"  │   Vessel Loss处理: CF图(Frangi+取反) / FA图(Frangi)")
        print(f"  │   Dice Loss: 专注血管区域重叠度（解决稀疏图背景主导问题）")
    elif is_cfoct:
        print(f"  │   Gamma: CF图=0.015, OCT图=0.02 (CF_OCT数据集，仅用于Vessel Loss)")
        print(f"  │   Scribble输入: 绿色通道 + {('取反' if args.mode == 'oct2cf' else '不取反')} + CLAHE")
        print(f"  │   Vessel Loss处理: CF图(Frangi+取反) / OCT图(Frangi+取反)")
        print(f"  │   Tile: 直接使用原始彩色图")
        print(f"  │   Dice Loss: 专注血管区域重叠度（解决稀疏图背景主导问题）")
    else:
        print(f"  │   Gamma: CF图=0.008, OCTA图=0.1 (CF-OCTA数据集，仅用于Vessel Loss)")
        print(f"  │   Scribble输入: 绿色通道 + {('取反' if args.mode == 'cf2octa' else '不取反')} + CLAHE")
        print(f"  │   Vessel Loss处理: CF图(Frangi+取反) / OCTA图(Frangi)")
        print(f"  │   Tile: 直接使用原始彩色图")
        print(f"  │   Dice Loss: 专注血管区域重叠度（解决稀疏图背景主导问题）")
    print(f"  ├─ 固定损失权重:")
    print(f"  │   MSE (噪声空间，GT-only蒙版): 1.0 (固定)")
    print(f"  │   MS-SSIM (像素空间，GT-only蒙版): {args.msssim_lambda} (固定)")
    print(f"  │   Vessel (像素空间，GT-only蒙版): {args.vessel_lambda} (固定)")
    print(f"  │   Gradient (像素空间，GT-only蒙版): {args.grad_lambda} (固定)")
    if args.bg_smooth_lambda > 0:
        print(f"  │   BG_Smooth (像素空间，GT黑边): {args.bg_smooth_lambda} (方案B)")
    print(f"  ├─ 蒙版策略: 所有损失只在GT有效区域监督（GT-only）")
    if args.dynamiclr:
        print(f"  ├─ 学习率: 动态衰减 (step<4000: 5e-5, step>=4000: Cosine 5e-5→1e-5)")
    else:
        print(f"  ├─ 学习率: 5e-5 (固定，无衰减)")
    print(f"  ├─ 早停机制: 验证损失连续 {patience} 次未提升则停止 【v10-3优化】")
    print(f"  │   Patience: {patience} 次（{patience * validate_every} 步）")
    print(f"  │   Warm-up: {min_train_steps} 步（此前不触发早停）")
    print(f"  ├─ 验证频率: 每 {validate_every} 步（固定验证子集10个样本）【v10-3优化】")
    print(f"  │   第一次验证随机选择子集，后续验证重复使用")
    print(f"  ├─ 检查点保存策略:")
    print(f"  │   - best_checkpoint: 保存验证损失最低的模型")
    print(f"  │   - latest_checkpoint: 每次验证覆盖保存（防止训练中断）")
    print(f"  │   - step_XXX/: 仅保存推理测试图像（不保存权重）")
    print(f"  └─ 训练日志: 每100步记录到 training_log.txt")
    print(f"\n  精度: FP32")
    print(f"  优化器: AdamW")
    ...
    while global_step < max_steps:
        ...
        for batch_data in train_loader:
            ...
            if global_step == 0:
                ...
                print(f"  1. {cond_filename}_scribble_input.png - Scribble ControlNet 输入（【v11】绿色通道+可选取反+CLAHE）")
            ...
            # 计算总损失（返回各分量用于日志记录）
            loss, loss_mse, loss_msssim, loss_vessel, loss_grad, loss_bg_smooth = compute_total_loss(
                noise_pred, noise, noisy_latents, latents, timesteps,
                args, noise_scheduler, vae_sf, msssim_loss_fn, device,
            ...
            if args.bg_smooth_lambda > 0:
                bg_smooth_accumulator.append(loss_bg_smooth.item())
            global_step += 1
            ...
            if global_step % 100 == 0:
                ...
                if args.bg_smooth_lambda > 0 and len(bg_smooth_accumulator) > 0:
                    avg_bg_smooth = np.mean(bg_smooth_accumulator)
                    bg_smooth_accumulator = []
                else:
                    avg_bg_smooth = 0.0
                ...
                # 构建日志消息
                msg_parts = [
                    f"[SD15-v12] step {global_step}/{max_steps}",
                    f"lr:{current_lr:.2e}",
                    f"mse:{avg_mse:.4f}",
                ]
                ...
                if args.bg_smooth_lambda > 0:
                    msg_parts.append(f"bg_smooth:{avg_bg_smooth:.4f}(λ={args.bg_smooth_lambda})")
    ...
    # ============ 最终保存 ============
    # 不再保存最终模型到根目录，只保留 best_checkpoint 和 latest_checkpoint
    # latest_checkpoint 已在最后一次验证时保存
    ...
    print(f"\n  【最佳模型信息 (Best Checkpoint)】")
    print(f"  ├─ 最佳步数: {best_step}")
    print(f"  ├─ 最佳验证损失: {best_val_loss:.6f}")
    print(f"  ├─ 保存路径: {best_ckpt_dir}")
    print(f"  └─ 包含文件: controlnet_scribble/, controlnet_tile/, optimizer.pt, best_info.txt")
    
    print(f"\n  【最新模型信息 (Latest Checkpoint)】")
    print(f"  ├─ 最新步数: {global_step}")
    print(f"  ├─ 保存路径: {latest_ckpt_dir}")
    print(f"  └─ 包含文件: controlnet_scribble/, controlnet_tile/, optimizer.pt, latest_info.txt")
    print(f"  ℹ️  latest_info.txt 中记录了保存时的步数和验证损失")
    
    print(f"\n  【训练配置】")
    print(f"  ├─ 模型版本: SD 1.5 + Dual ControlNet v12 (Scribble + Tile, FP32)")
    print(f"  ├─ 数据集类型: {'CF-FA' if is_cffa else ('CF_OCT' if is_cfoct else 'CF-OCTA')}")
    print(f"  ├─ v12 新特性: GT-only监督（只在GT有效区域监督，零监督区域依靠条件引导）")
    print(f"  ├─ v12 推理优化: CFG={args.cfg_scale}（降低外推保守性，推荐5.0-7.0）")
    print(f"  ├─ v11 特性: Scribble输入改用绿色通道（避免Frangi边界伪影）")
    print(f"  ├─ v10-3 特性: Vessel Loss 使用 Dice Loss（解决稀疏血管图问题）")
    print(f"  ├─ v10-3 早停优化: 固定验证子集 + Patience={patience} + Warm-up={min_train_steps}")
    print(f"  ├─ v10-2 特性: 固定学习率 5e-5 + 早停机制")
    print(f"  ├─ v10 特性: 统一数据加载器 + gen_mask侵蚀掩码（仅用于Vessel Loss）")
    ...
    if args.bg_smooth_lambda > 0:
        print(f"  ├─ 损失函数: MSE(噪声空间) + MS-SSIM(λ={args.msssim_lambda}) + Vessel(λ={args.vessel_lambda}) + Grad(λ={args.grad_lambda}) + BG_Smooth(λ={args.bg_smooth_lambda})")
    else:
    ...
    if args.dynamiclr:
        ...
        print(f"  ├─ 学习率: 5e-5 (固定，无衰减)")
    print(f"  ├─ 验证频率: 每 {validate_every} 步（固定验证子集）")
    print(f"  ├─ 早停耐心: {patience} 次（{patience * validate_every} 步）")
    print(f"  └─ Warm-up: {min_train_steps} 步（此前不触发早停）")
