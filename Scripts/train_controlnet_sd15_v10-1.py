# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v10-1
...
【v10-1 更新】✨
1. **Tile 条件图自动匹配目标域风格**
   - cf2fa / cf2oct：提取 CF 绿色通道并增强暗血管对比
   - cf2octa：绿色通道取反后增强亮血管（匹配 OCTA 外观）
   - data_loader_all_v2 中的 stylize_tile_image 训练/推理统一生效
...
- **【v8-3-2更新】CF训练集改用彩色原图；v10-1 中 Tile 输入依据目标域自动风格化**
...
# 导入统一数据加载器（v10-1）
from data_loader_all_v2 import (
...
# 注意：图像处理参数配置已移至 data_loader_all_v2.py
...
def main():
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v10-1】✨")
    print("="*70)
    ...
    print(f"\n  【v10-1 更新】✨")
    print(f"  ├─ Tile 条件图自动匹配目标域风格")
    print(f"  │   cf2fa/cf2oct: CF绿色通道对比增强，突出暗血管")
    print(f"  │   cf2octa     : CF绿色通道取反后对比增强，突出亮血管")
    print(f"  │   训练与推理统一使用 stylize_tile_image")
    print(f"\n  【v10 更新】✨")
    print(f"  ├─ 使用统一数据加载器（data_loader_all_v2.py）")
    print(f"  │   整合三个数据集加载器（CF-OCTA、CF-FA、CF_OCT）")
    print(f"  ├─ Single Source of Truth（单一数据源）")
    print(f"  │   所有图像处理参数在 data_loader_all_v2.py 统一管理")
    ...
    if not is_cffa:
        ...
        print(f"  │   Tile输入: cf2octa 使用绿色通道取反+对比增强匹配亮血管")
    ...
    if is_cffa:
        ...
        print(f"  │   Tile: cf2fa 使用绿色通道对比增强，fa2cf 保留原图")
    elif is_cfoct:
        ...
        print(f"  │   Tile: cf2oct 使用绿色通道对比增强，oct2cf 保留原图")
    else:
        ...
        print(f"  │   Tile: cf2octa 使用绿色通道取反增强，octa2cf 保留原图")
    print(f"  ├─ 固定损失权重:")
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            if global_step == 0:
                ...
                # 【v10-1 新增】保存 CF 原图和风格化对比（仅正向任务）
                if args.mode in ["cf2fa", "cf2oct", "cf2octa"]:
                    try:
                        # 加载 CF 原图
                        cf_original_pil = Image.open(cond_paths[0]).convert("RGB")
                        
                        # 导入风格化函数
                        from data_loader_all_v2 import stylize_tile_image
                        
                        # 应用风格化处理（对比度增强）
                        cf_stylized_pil = stylize_tile_image(cf_original_pil, args.mode)
                        
                        # 保存 CF 原图（resize 到 512×512 便于对比）
                        cf_original_512 = cf_original_pil.resize((SIZE, SIZE))
                        cf_original_512.save(os.path.join(debug_dir, f"{cond_filename}_00_cf_original.png"))
                        
                        # 保存风格化后的 CF 图（resize 到 512×512）
                        cf_stylized_512 = cf_stylized_pil.resize((SIZE, SIZE))
                        cf_stylized_512.save(os.path.join(debug_dir, f"{cond_filename}_01_cf_stylized.png"))
                        
                        print(f"\n{'='*70}")
                        print(f"✓ CF 原图和风格化对比已保存:")
                        print(f"  - {cond_filename}_00_cf_original.png: CF 原始彩色图")
                        print(f"  - {cond_filename}_01_cf_stylized.png: 风格化后的 Tile 输入（对比度增强）")
                        print(f"{'='*70}\n")
                    except Exception as e:
                        print(f"  ⚠ 保存 CF 原图和风格化对比失败: {e}")
                
                # 1. 保存Scribble条件图（Vessel）
                cond_scribble_save = (cond_scribble[0].cpu().float().permute(1, 2, 0).numpy() * 255).clip(0, 255).astype(np.uint8)
                Image.fromarray(cond_scribble_save).save(os.path.join(debug_dir, f"{cond_filename}_02_scribble_input.png"))
                
                # 2. 保存Tile条件图（最终送入模型的）
                cond_tile_save = (cond_tile[0].cpu().float().permute(1, 2, 0).numpy() * 255).clip(0, 255).astype(np.uint8)
                Image.fromarray(cond_tile_save).save(os.path.join(debug_dir, f"{cond_filename}_03_tile_input_final.png"))
                
                # 3. 保存配准后的目标图
                tgt_save = ((tgt[0].cpu().float().permute(1, 2, 0).numpy() + 1) / 2 * 255).clip(0, 255).astype(np.uint8)
                Image.fromarray(tgt_save).save(os.path.join(debug_dir, f"{tgt_filename}_04_target_registered.png"))
                
                print(f"\n{'='*70}")
                print(f"✓ Step 0 调试图像已保存到: {debug_dir}")
                if args.mode in ["cf2fa", "cf2oct", "cf2octa"]:
                    print(f"  0. {cond_filename}_00_cf_original.png - CF 原始彩色图")
                    print(f"  1. {cond_filename}_01_cf_stylized.png - 风格化后的 CF 图（对比度增强）")
                print(f"  2. {cond_filename}_02_scribble_input.png - Scribble ControlNet 输入（Vessel血管图）")
                print(f"  3. {cond_filename}_03_tile_input_final.png - Tile ControlNet 最终输入")
                print(f"  4. {tgt_filename}_04_target_registered.png - 配准后目标图")
                print(f"{'='*70}\n")
    ...
    print(f"  模型版本: SD 1.5 + Dual ControlNet v10-1 (Scribble + Tile, FP32)")
    print(f"  数据集类型: {'CF-FA' if is_cffa else ('CF_OCT' if is_cfoct else 'CF-OCTA')}")
    print(f"  v10-1 特性: Tile 条件图目标域风格化 + 统一数据加载器 + gen_mask侵蚀掩码")

    if is_cffa:
        print(f"  Tile 条件图: cf2fa 使用绿色通道对比增强，fa2cf 保留原图")
    elif is_cfoct:
        print(f"  Tile 条件图: cf2oct 使用绿色通道对比增强，oct2cf 保留原图")
    else:
        print(f"  Tile 条件图: cf2octa 使用绿色通道取反增强，octa2cf 保留原图")
