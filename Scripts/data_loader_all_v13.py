...
from registration_cf_oct_v2 import register_cfoct_pair
...
def preprocess_for_vessel_extraction(img_pil, mode, dataset_type='CFFA'):
    ...
    # ============ 图像预处理 ============
    if is_cfoct:
        # CF_OCT 特殊处理：直接 resize 到 512×512（不使用 resize_with_padding）
...
class UnifiedDataset(Dataset):
    ...
    def _apply_registration_cfoct(self, cond_pil, tgt_pil, affine_data):
        """
        CF_OCT 配准逻辑（v2方案：使用透视变换）
        
        【配准策略】
        - 不管 cf2oct 还是 oct2cf，统一配准到 CF 域
        - 配准后直接 resize 到 512×512
        """
        ...
        if cf_pts_path and oct_pts_path and os.path.exists(cf_pts_path) and os.path.exists(oct_pts_path):
            # 使用 register_cfoct_pair 统一配准接口
            # 不管什么模式，统一配准到 CF 域
            cond_np, tgt_np = register_cfoct_pair(
                cf_img=cf_pil,
                oct_img=oct_pil,
            ...
            cond_pil_processed = Image.fromarray(cond_np)
            tgt_pil_processed = Image.fromarray(tgt_np)
        else:
            # 没有关键点文件时，两图都直接 resize
            cond_resized = cv2.resize(
                np.array(cond_pil), 
                (SIZE, SIZE),
                interpolation=cv2.INTER_CUBIC
            )
            tgt_resized = cv2.resize(
                np.array(tgt_pil), 
                (SIZE, SIZE),
                interpolation=cv2.INTER_CUBIC
            )
            cond_pil_processed = Image.fromarray(cond_resized)
            tgt_pil_processed = Image.fromarray(tgt_resized)
