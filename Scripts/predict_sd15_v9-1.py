...
from registration_cf_oct import (  # CF_OCT 配准工具（归一化方法）
    load_keypoints as load_keypoints_cfoct, 
...
GAMMA_CFOCT_OCT = 0.05     # OCT图

# Frangi 滤波通用参数
FRANGI_SIGMAS = range(1, 16)  # 多尺度检测范围

# ============ 参数解析 ============
parser = argparse.ArgumentParser(description="SD 1.5 + Dual ControlNet 推理脚本 v9 (支持 CF-OCTA、CF-FA 和 CF_OCT)")
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + Dual ControlNet 推理参数 (v9 双路架构 - 所有模式)\n")
    f.write("="*70 + "\n")
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        # v9: ControlNet 预处理 - 支持 CF-FA、CF-OCTA 和 CF_OCT
        if is_cffa:
            # CF-FA 模式：双路 ControlNet (Scribble + Tile)
            ...
            # CF_OCT 模式：双路 ControlNet (Scribble + Tile)
            # 1. Scribble 输入：Vessel 血管图（CF和OCT血管都是暗色，都需要取反）
            from data_loader_cfoct_v2 import extract_vessel_hessian
            image_type = 'cf' if args.mode == "cf2oct" else 'oct'
        ...
        if is_cfoct:
            ...
            # 2. 应用黑边蒙版到预测图
            # 检测 tile 输入图中的黑边区域（配准产生的边缘）
            cond_tile_np = np.array(cond_tile)
            
            # 检测纯黑色像素（所有通道都为0）
            black_mask = np.all(cond_tile_np == 0, axis=2)  # (H, W) boolean
            
            # 将预测图转换为numpy数组
            pred_np_masked = np.array(img).copy()
            
            # 将黑边区域设置为纯黑色
            pred_np_masked[black_mask] = [0, 0, 0]
            ...
            # 可选：保存未应用蒙版的原始预测图（用于调试）
            # img.save(os.path.join(img_out_dir, "pred_no_mask.png"))
        ...
        try:
            ...
            if is_cffa:
                ...
                # CF_OCT 模式：使用归一化关键点方法
                if need_register and isinstance(affine_path, tuple):
                    cond_pts_path, tgt_pts_path = affine_path
                    
                    if cond_pts_path and tgt_pts_path and os.path.exists(cond_pts_path) and os.path.exists(tgt_pts_path):
                        # 加载关键点
                        cond_points = load_keypoints_cfoct(cond_pts_path)
                        tgt_points = load_keypoints_cfoct(tgt_pts_path)
                        
                        # 获取原始图像尺寸
                        cond_w, cond_h = src_img_original.size
                        tgt_w, tgt_h = target_img_preprocessed.size
                        ...
                        target_img_512 = target_img_preprocessed.resize((SIZE, SIZE))
            ...
            if is_cfoct:
                ...
                # CF-FA 和 CF-OCTA 模式：保存原尺寸的配准图
                target_img_registered.save(os.path.join(img_out_dir, "target_gt.png"))
            
            # 4. 生成并保存棋盘图
            try:
                if is_cfoct:
                    # CF_OCT 模式：使用 512×512 的 4×4 棋盘（使用蒙版后的预测图）
...
print(f"  架构: 双路 ControlNet (Scribble + Tile) - v9")
print(f"  数据集: {'CF-FA' if is_cffa else ('CF_OCT' if is_cfoct else 'CF-OCTA')}")
