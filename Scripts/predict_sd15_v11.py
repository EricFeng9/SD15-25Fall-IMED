...
- FOV/掩码参数统一化：使用 get_image_params() 从 data_loader_all.py 获取
...
with open(log_path, "a") as f:
    ...
    f.write("  - FOV/掩码参数统一化: 使用 get_image_params() 从 data_loader_all.py 获取\n")
...
print(f"    - FOV/掩码参数统一化: 使用 get_image_params() 从 data_loader_all.py 获取")
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        if is_cfoct:
            ...
            # 2. 【v9-3】应用智能黑边蒙版到预测图
            # 【v11修复】mask也需要使用 resize_with_padding，与输入图像保持一致的空间对应关系
            mask_512, _, _, _ = resize_with_padding(
                mask_original, 
