"""
SD 1.5 ControlNet 推理脚本 - v10-1

【v10-1 更新】Tile 条件图目标域风格化
- cf2fa / cf2oct：CF绿色通道对比增强以匹配暗血管目标
- cf2octa       ：CF绿色通道取反并增强亮血管结构
- 训练/推理统一启用（data_loader_all_v2.stylize_tile_image）
...
- CF-OCTA 数据集更新：
  * CF 训练集已改为彩色原图
  * Tile 输入：v10-1 将 cf2octa 的 CF 图做绿色通道取反及对比增强
  * 目标图：直接使用彩色原图（不做预处理）
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + Dual ControlNet 推理参数 (v10-1 双路架构 - 所有模式)\n")
    f.write("="*70 + "\n")
    ...
    f.write("【v10-1更新】Tile 条件图目标域风格化:\n")
    f.write("  - cf2fa/cf2oct: CF绿色通道对比增强，突出暗血管\n")
    f.write("  - cf2octa     : CF绿色通道取反+对比增强，突出亮血管\n")
    f.write("  - 推理阶段与训练保持一致\n")
    f.write("【v10更新】统一数据加载器 + FOV圆形掩码:\n")
    f.write("  - 使用统一数据加载器（data_loader_all_v2.py）\n")
...
print(f"  【v10-1更新】Tile 条件图目标域风格化:")
print(f"    - cf2fa/cf2oct: CF绿色通道对比增强，突出暗血管")
print(f"    - cf2octa     : CF绿色通道取反+对比增强，突出亮血管")
print(f"    - 推理阶段与训练保持一致")
