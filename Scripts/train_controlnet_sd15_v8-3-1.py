# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v8-3-1

【基于】train_controlnet_sd15_v8-3.py
...
- **【v8-3-1更新】所有模式统一使用Frangi血管滤波 + L1距离损失**
- 不同数据集使用不同的gamma值优化血管检测
- FP32 训练

【v8-3-1 更新】✨
1. **血管结构损失 (Vessel Loss) - 所有模式启用**
   ...
   - CF-OCTA数据集：sigmas=range(1,16)
     * CF图像：gamma=0.008（训练集灰度图，推理时绿色通道+取反）
...
python train_controlnet_sd15_v8-3-1.py --mode cf2fa --name sd15_v8-3-1_cffa --max_steps 8000 --scribble_scale 0.8 --vessel_lambda 0.05

# CF-OCTA 训练（双路Vessel+Tile模式，gamma_cf=0.008, gamma_octa=0.1）
python train_controlnet_sd15_v8-3-1.py --mode cf2octa --name sd15_v8-3-1_cfocta --max_steps 8000 --scribble_scale 0.8 --vessel_lambda 0.05
...
def compute_vessel_loss_frangi(pred_imgs, gt_imgs, mode='cf2fa', sigmas=range(1, 16), 
                                alpha=0.5, beta=0.5, gamma_cffa=0.015, gamma_cf=0.008, gamma_octa=0.1, 
                                debug_dir=None):
    """
    ...
    - CF-FA 数据集:
      * CF图: 绿色通道 + 取反（血管是暗色）
      * FA图: 绿色通道 + 不取反（血管是亮色）
    - CF-OCTA 数据集（VAE解码后都是亮色血管）:
      * CF图: 提取绿色通道，不取反（训练集已处理）
      * OCTA图: 提取绿色通道，不取反
    
    参数:
        pred_imgs: 预测图像 (B, 3, H, W)，范围 [-1, 1]
        gt_imgs: 目标图像 (B, 3, H, W)，范围 [-1, 1]
        mode: 训练模式 ('cf2fa', 'fa2cf', 'cf2octa', 'octa2cf')
        sigmas: Frangi 多尺度参数（默认 range(1, 16)）
        alpha: Frangi 板状结构敏感度（默认 0.5）
        beta: Frangi 球状结构敏感度（默认 0.5）
        gamma_cffa: Frangi 噪声抑制阈值（CF-FA模式使用，默认 0.015）
        gamma_cf: CF图的噪声抑制阈值（CF-OCTA模式使用，默认 0.008）
        gamma_octa: OCTA图的噪声抑制阈值（CF-OCTA模式使用，默认 0.1）
    ...
    if is_cffa:
        ...
        # CF-OCTA 数据集：VAE解码后都是亮色血管，不需要取反
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    """
    运行推理测试（每500步）- Dual ControlNet 版本 v8-3-1 (Scribble + Tile)
    ...
    if is_cffa:
        ...
        # 判断图像类型和预处理方式
        if mode == "cf2octa":
            # CF图（推理时是彩色）：提取绿色通道 + 取反
            src_array = np.array(src_img_original)
            green_channel = src_array[:, :, 1]  # 提取绿色通道
            green_inverted = 255 - green_channel  # 取反（血管是暗色）
            src_preprocessed = Image.fromarray(green_inverted).convert("RGB")
            image_type = 'cf'
        else:  # octa2cf
            # OCTA图：直接使用
            src_preprocessed = src_img_original
            image_type = 'octa'
    ...
    if is_cffa:
        # CF-FA 模式：使用原始彩色图（保留颜色和细节信息）
        cond_tile_pil = src_img_original.resize((SIZE, SIZE))
    else:
        # CF-OCTA 模式：使用预处理后的图像（与训练时一致）
        if mode == "cf2octa":
            # cf2octa: 使用绿色通道+取反后的图像（训练集是灰度图）
            cond_tile_pil = src_preprocessed.resize((SIZE, SIZE))
        else:  # octa2cf
            # octa2cf: 直接使用原始 OCTA 图
            cond_tile_pil = src_img_original.resize((SIZE, SIZE))
...
def main():
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v8-3-1】✨")
    print("="*70)
    ...
    if is_cffa:
        ...
        print(f"  │   图像处理: 训练集(灰度) / 推理时CF图(绿色通道+反转)")
    print(f"  ├─ 固定损失权重:")
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            loss_vessel = compute_vessel_loss_frangi(
                ...
                gamma_cf=0.008,    # CF-OCTA模式，CF图使用
                gamma_octa=0.1,  # CF-OCTA模式，OCTA图使用
            ...
            if global_step % 100 == 0:
                ...
                # 构建日志消息
                msg_parts = [
                    f"[SD15-v8-3-1] step {global_step}/{max_steps}",
                    f"lr:{current_lr:.2e}",
                    f"mse:{avg_mse:.4f}",
                ]
    ...
    print(f"  模型版本: SD 1.5 + Dual ControlNet v8-3-1 (Scribble + Tile, FP32)")
    print(f"  数据集类型: {'CF-FA' if is_cffa else 'CF-OCTA'}")
