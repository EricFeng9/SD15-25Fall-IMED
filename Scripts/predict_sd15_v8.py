"""
SD 1.5 ControlNet 推理脚本 - v8

【v8 更新】基于 v7，根据数据集类型使用不同架构
- 架构变化：
  * CF-FA 模式：双路 ControlNet（Scribble + Tile）
    - Scribble: Vessel血管图引导
    - Tile: 原图细节保留
  * CF-OCTA 模式：单路 Tile ControlNet
    - 只使用 Tile ControlNet
    - 完全移除 HED 边缘检测相关代码
- 支持 4 种推理模式：
  * cf2octa / octa2cf: 使用 CF-OCTA 数据集（单路 Tile）
  * cf2fa / fa2cf: 使用 CF-FA 数据集（双路 Scribble+Tile）
- 输出结构优化：
  * 每张图片独立文件夹
  * 包含：原图、预测图（pred）、配准目标图（GT）、棋盘对比图
- CF-FA 使用关键点实时计算配准矩阵
- CF-OCTA 使用预计算的仿射矩阵
...
- 根据数据集类型自适应架构：
  ...
  * CF-OCTA: 单路 Tile ControlNet（无HED，纯原图细节保留）
- 智能输出结构（每张图独立文件夹）：
  * 原图（原尺寸）
  * 预测图（pred，原尺寸）
  * 配准目标图（GT，原尺寸）
...
【使用说明】
基础用法：
  # CF-OCTA 推理（单路 Tile ControlNet）
  python predict_sd15_v7.py --mode cf2octa --name sd15_v8-3_cfocta
  
  # CF-FA 推理（双路 Scribble+Tile）
  python predict_sd15_v7.py --mode cf2fa --name sd15_v8-3_cffa

完整参数示例（CF-FA 双路模式）：
  python predict_sd15_v7.py \\
    --mode cf2fa \\
    --name sd15_v8-3_cffa \\
    --step 8000 \\
...
ControlNet 参数（可调节生成质量）：
  --scribble_scale  Scribble ControlNet 条件强度，范围 0.0-2.0（默认：0.8）
                    - 仅 CF-FA 模式有效：控制血管结构引导强度
                    - CF-OCTA 模式：忽略此参数（不使用 Scribble ControlNet）
                    - 越大：越强调血管结构
...
2. 使用特定步数的权重：
   python predict_sd15_v7.py --mode cf2fa --name sd15_v8-3_cffa --step 8000

3. 使用自定义保存目录：
   python predict_sd15_v7.py --mode cf2fa --name sd15_v8-3_cffa --step 8000 --savedir test_results

4. 调整 ControlNet 强度（CF-FA 双路模式）：
   python predict_sd15_v7.py --mode cf2fa --name sd15_v8-3_cffa \\
     --scribble_scale 0.8 --tile_scale 1.0 --steps 50 --seed 42

5. 调整 ControlNet 强度（CF-OCTA 单路模式）：
   python predict_sd15_v7.py --mode cf2octa --name sd15_v8-3_cfocta \\
     --tile_scale 1.0 --steps 50 --seed 42
...
7. 反向任务（FA→CF / OCTA→CF）：
   python predict_sd15_v7.py --mode fa2cf --name sd15_v8-3_cffa --step 8000
   python predict_sd15_v7.py --mode octa2cf --name sd15_v8-3_cfocta --step 8000
...
from chessboard import chessboard_gen_720576  # 棋盘图生成
...
# ============ 参数解析 ============
parser = argparse.ArgumentParser(description="SD 1.5 + Dual ControlNet 推理脚本 v8 (支持 CF-OCTA 和 CF-FA)")
...
parser.add_argument("--scribble_scale", type=float, default=0.8,
                    help="Scribble ControlNet 条件强度 (0.0-2.0，CF-OCTA模式自动设为0)")
...
if is_cffa:
    ...
    if not os.path.isdir(ctrl_scribble_dir) or not os.path.isdir(ctrl_tile_dir):
        raise FileNotFoundError(
            f"未找到 Dual ControlNet 子目录 (CF-FA 模式需要):\n"
    ...
    # CF-OCTA 模式：只需要 Tile ControlNet
    if not os.path.isdir(ctrl_tile_dir):
        raise FileNotFoundError(
            f"未找到 Tile ControlNet 子目录 (CF-OCTA 模式需要):\n"
            f"  Tile: {ctrl_tile_dir}\n"
            f"请确认使用的是包含 Tile ControlNet 的模型"
        )
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    if is_cffa:
        f.write("SD 1.5 + Dual ControlNet 推理参数 (v8 双路架构 - CF-FA)\n")
    else:
        f.write("SD 1.5 + 单路 Tile ControlNet 推理参数 (v8 - CF-OCTA)\n")
    f.write("="*70 + "\n")
    ...
    if is_cffa:
        f.write("【v8架构】双路 ControlNet: Scribble(Vessel) + Tile\n")
        f.write(f"【CF-FA】原始尺寸: {CFFA_ORIGINAL_SIZE[0]}×{CFFA_ORIGINAL_SIZE[1]}\n")
    else:
        f.write("【v8架构】单路 Tile ControlNet（无HED）\n")
        f.write("【CF-OCTA】原始尺寸: 400×400\n")
    f.write("="*70 + "\n\n")

print("\n" + "="*70)
if is_cffa:
    print("正在加载 SD 1.5 + Dual ControlNet 模型 (CF-FA 模式)...")
else:
    print("正在加载 SD 1.5 + 单路 Tile ControlNet 模型 (CF-OCTA 模式)...")
print("="*70)
...
if is_cffa:
    print(f"  【v8架构】双路 ControlNet (Scribble + Tile)")
    print(f"  【预处理】Scribble输入: Vessel血管图 / Tile输入: 原图")
else:
    print(f"  【v8架构】单路 Tile ControlNet")
    print(f"  【预处理】Tile输入: 原图")

# ============ 加载模型组件（与训练脚本一致）============
dtype = torch.float16 if args.use_fp16 else torch.float32
device = torch.device("cuda")

# 根据数据集类型加载 ControlNet
if is_cffa:
    # CF-FA 模式：加载双路 ControlNet (Scribble + Tile)
    ...
    # CF-OCTA 模式：只加载单路 Tile ControlNet
    controlnet_tile = ControlNetModel.from_pretrained(
        ctrl_tile_dir, 
        torch_dtype=dtype, 
        local_files_only=True
    ...
    # 显式设置为 eval 模式
    controlnet_tile.eval()
    print("✓ 单路 Tile ControlNet 已设置为 eval 模式")
...
# 根据数据集类型构建 Pipeline
if is_cffa:
    # CF-FA 模式：使用 MultiControlNet (Scribble + Tile)
    ...
    # CF-OCTA 模式：使用单路 Tile ControlNet
    print("✓ 使用单路 Tile ControlNet")
...
if is_cffa:
    print(f"  【v8架构】双路 ControlNet: Scribble(Vessel) + Tile")
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        with torch.no_grad():
            if is_cffa:
                # CF-FA 模式：双路 ControlNet 推理 (Scribble + Tile)
                img = pipe(
                    prompt=args.prompt,
                ...
                # CF-OCTA 模式：单路 Tile ControlNet 推理
                img = pipe(
                    prompt=args.prompt,
                    negative_prompt=args.negative_prompt if args.negative_prompt else None,
        ...
        try:
            ...
            pred_np_original = np.array(pred_img_resized)  # 原尺寸预测图
            gt_np_original = np.array(target_img_registered)  # 原尺寸配准目标图
...
if is_cffa:
    print(f"  架构: 双路 ControlNet (Scribble + Tile) - v8")
else:
    print(f"  架构: 单路 Tile ControlNet - v8")
print(f"  数据集: {'CF-FA' if is_cffa else 'CF-OCTA'}")
...
if len(valid_metrics) > 0:
    ...
    with open(log_path, "a") as f:
        ...
        if is_cffa:
            f.write("    【v8架构】双路 ControlNet (Scribble + Tile)\n")
            f.write("    Scribble: Vessel血管引导\n")
        else:
            f.write("    【v8架构】单路 Tile ControlNet（无HED）\n")
