...
# 基本配置（v3-1：SD2.1-768 + ControlNet SD2.1 HED；条件统一为 HED）
...
out_root  = "/data/student/Fengjunming/SDXL_ControlNet/results/out_ctrl_sd21_v3_1"
...
class PairCSVHED(Dataset):
    """
    读取 CSV，每个样本返回 (cond_hed, tgt)：
    - cond_hed: 对原图做 HED 得到的软边缘（[0,1] RGB，SIZE×SIZE）
    - tgt     : 目标域图像，经线性缩放到 [-1,1]
    """
    def __init__(self, csv_path: str, mode: str, hed: HEDdetector):
        self.rows = []
        self.mode = mode
        self.hed = hed
    ...
    def __getitem__(self, idx: int):
        ...
        # 生成 HED 条件图（软边缘），转 RGB 再转张量
        hed_pil = self.hed(src_pil)
        cond = pil_to_tensor_rgb(hed_pil)
        # 与管线保持一致：ControlNet 条件按 [-1,1] 归一化
        cond = cond * 2 - 1
...
def _run_eval_and_log(eval_rows, step_dir, mode, metric_state, hed: HEDdetector):
...
def main():
    ...
    parser.add_argument("--name", default='v3_1_hed')
    ...
    vtag = args.name if args.name else 'v3_1_hed'
    out_dir = os.path.join(out_root, args.mode, vtag); os.makedirs(out_dir, exist_ok=True)

    # 初始化 HED（统一条件）
    hed = HEDdetector.from_pretrained('/data/student/Fengjunming/SDXL_ControlNet/models/hed', local_files_only=True)

    # 数据加载（仅训练集）
    train_ds = PairCSVHED(args.train_csv, mode=args.mode, hed=hed)
    ...
    t_block = time.time()
    print(f"\n[v3-1] 统一 HED 条件，开始训练... 模式: {args.mode} | 输出: {out_dir} | 训练CSV: {args.train_csv}")
    ...
    while global_step < max_steps:
        for cond, tgt in train_loader:
            ...
            if global_step % 100 == 0:
                ...
                print(f"[v3-1] step {global_step}/{max_steps} loss {loss.item():.4f} | 100step {elapsed:.2f}s ({elapsed/100:.3f}s/step)")
            ...
            if global_step % 1000 == 0:
                ...
                # 周期评估（统一 HED）
                print(f"[v3-1] 进行周期评估 @ step {global_step}")
                # 预载 eval 样本
                eval_rows = []
                try:
                    ...
                    eval_rows = []
                if eval_rows:
                    _run_eval_and_log(eval_rows, step_dir, args.mode, metric_state, hed)

    pipe.controlnet.save_pretrained(out_dir)
    print("[v3-1] saved to", out_dir)
    ...
    try:
        ...
        with open(args.eval_csv) as f:
            for i, r in enumerate(csv.DictReader(f)):
                ...
                eval_rows.append(r)
        if eval_rows:
            _run_eval_and_log(eval_rows, out_dir, args.mode, metric_state, hed)
    except Exception as e:
        pass
