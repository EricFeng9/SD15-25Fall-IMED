'''
训练脚本 - SD 1.5 + HED ControlNet 版本（v5）

【版本】v5-sd15
【基于】train_controlnet_sd21_v5.py (SD 2.1 版本)
...
- 尺寸: 768×768 → 512×512
- 添加 HED 边缘检测预处理
- 第一步输出调试图像（原图、配准图、HED边缘图）
- 每1000步推理测试（固定图像，cfg=7.5）

【使用方法】
训练: python train_controlnet_sd15_v5.py --mode cf2octa --name sd15_hed_v5 --max_steps 8000
恢复: python train_controlnet_sd15_v5.py --mode cf2octa --name sd15_hed_v5 --resume_from /path/to/step_6000
'''
...
ctrl_dir = "/data/student/Fengjunming/SDXL_ControlNet/models/controlnet-sd15-hed"
...
# 输出目录
out_root  = "/data/student/Fengjunming/SDXL_ControlNet/results/out_ctrl_sd15"
...
class PairCSV(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 统一resize到训练尺寸
        cond_hed = pil_to_tensor_rgb(cond_hed_pil)  # HED边缘图 [0,1]
        tgt = pil_to_tensor_rgb(tgt_pil)  # 目标图 [0,1]
        
        # ControlNet输入保持[0,1]，VAE需要[-1,1]
        tgt = tgt * 2 - 1
        
        # 返回 HED 边缘图 + 原图路径（用于调试）
        return cond_hed, tgt, cond_path, tgt_path
...
def run_inference_test(fixed_sample, step_dir, step_num, fixed_seed=42):
    """
    运行推理测试（每1000步）
    
    参数:
        fixed_sample: (cond_hed, tgt, cond_path, tgt_path) - 固定的测试样本
        step_dir: checkpoint 保存目录
        step_num: 当前步数
        fixed_seed: 固定的随机种子（用于对比不同step的训练效果）
    """
    print(f"\n{'='*70}")
    print(f"运行推理测试 (step {step_num})...")
    print(f"{'='*70}")
    
    cond_hed, tgt, cond_path, tgt_path = fixed_sample
    ...
    # 保存条件图（HED边缘图）
    cond_hed_save = (cond_hed.cpu().float().permute(1, 2, 0).numpy() * 255).clip(0, 255).astype(np.uint8)
    Image.fromarray(cond_hed_save).save(os.path.join(infer_dir, "condition_hed.png"))
    ...
    # 准备输入
    cond_hed_pil = Image.fromarray(cond_hed_save)
    ...
    pred_img.save(os.path.join(infer_dir, f"prediction_step{step_num}.png"))
    
    # 保存参考目标图
    tgt_save = ((tgt.cpu().float().permute(1, 2, 0).numpy() + 1) / 2 * 255).clip(0, 255).astype(np.uint8)
    Image.fromarray(tgt_save).save(os.path.join(infer_dir, "target_reference.png"))
    
    print(f"✓ 推理测试完成，结果保存至: {infer_dir}")
    print(f"  条件图路径: {cond_path}")
    print(f"  目标图路径: {tgt_path}")
...
def main():
    ...
    # ============ 准备固定的推理测试样本 ============
    # 从训练集中随机选一个样本（固定用于所有推理测试）
    import random
    random.seed(42)  # 固定随机种子，确保每次运行选同一个样本
    fixed_sample_idx = random.randint(0, len(train_ds) - 1)
    fixed_sample = train_ds[fixed_sample_idx]
    print(f"\n固定推理测试样本 (索引 {fixed_sample_idx}):")
    print(f"  条件图: {fixed_sample[2]}")
    print(f"  目标图: {fixed_sample[3]}\n")
    ...
    print("\n" + "="*70)
    print("正在加载 Stable Diffusion 1.5 + HED ControlNet 模型...")
    print("="*70)
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + HED ControlNet 训练配置】")
    print("="*70)
    print(f"  模型: Stable Diffusion 1.5 (512×512)")
    print(f"  ControlNet: HED (边缘检测预处理，适合医学图像)")
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            cond_hed, tgt, cond_paths, tgt_paths = batch_data
            cond_hed = cond_hed.to(device)
            tgt = tgt.to(device)
            b = tgt.shape[0]
            
            # 第一步保存调试图像（原图、配准图、HED边缘图）
            if global_step == 0:
                ...
                # 1. 保存原始条件图（CF 或 OCTA）
                cond_original_pil = Image.open(cond_paths[0]).convert("RGB").resize((SIZE, SIZE))
                cond_original_pil.save(os.path.join(debug_dir, f"{cf_filename}_original.png"))
                ...
                print(f"  3. {cf_filename}_hed_edge.png - HED边缘图 (ControlNet输入)")
                print(f"{'='*70}\n")
            ...
            # ControlNet 前向传播（SD 1.5 简化版）
            down_samples, mid_sample = controlnet(
                noisy_latents,
                timesteps,
                encoder_hidden_states=prompt_embeds,
                controlnet_cond=cond_hed,  # 使用 HED 边缘图
                return_dict=False
            )
            
            # UNet 预测噪声（SD 1.5 简化版）
            noise_pred = unet(
                noisy_latents,
                timesteps,
                encoder_hidden_states=prompt_embeds,
            ...
            if global_step % 100 == 0:
                ...
                msg = (f"[SD15-v5] step {global_step}/{max_steps} | "
            ...
            if global_step % 1000 == 0:
                ...
                # 运行推理测试（固定样本，cfg=7.5）
                run_inference_test(fixed_sample, step_save_dir, global_step)
    ...
    print(f"  模型: SD 1.5 + HED ControlNet (FP32)")
    if args.resume_from:
