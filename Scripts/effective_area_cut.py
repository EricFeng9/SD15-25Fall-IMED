...
def filter_valid_area(img1, img2):
    ...
    print(f"原始有效区域像素数: {valid_pixel_count}/{total_pixels} ({valid_ratio:.2f}%)")
    ...
    print(f"有效区域边界: 行[{row_min}:{row_max+1}], 列[{col_min}:{col_max+1}]")
    print(f"裁剪后尺寸: {row_max+1-row_min} x {col_max+1-col_min}")
