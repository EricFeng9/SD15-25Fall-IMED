# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v8

【基于】train_controlnet_sd15_v7.py
...
【v8 更新】
1. 双路 ControlNet 架构
   - Scribble ControlNet: 血管结构引导（CF-FA）
   - Tile ControlNet: 原图细节保留（所有模式）
   - CF-OCTA模式：Scribble权重自动设为0（保持单路Tile行为）

2. 支持 4 种训练模式
   - cf2octa: CF → OCTA (HED+Tile，Scribble权重=0)
   - octa2cf: OCTA → CF (HED+Tile，Scribble权重=0)
   - cf2fa: CF → FA (Vessel+Tile，双路生效)
   - fa2cf: FA → CF (Vessel+Tile，双路生效)

3. 数据加载器
   - CF-FA: data_loader_cffa_v2 (Vessel血管提取)
   - CF-OCTA: data_loader_cfocta (HED边缘检测)

【使用方法】
# CF-OCTA 训练（单路Tile模式，Scribble权重=0）
python train_controlnet_sd15_v8.py --mode cf2octa --name sd15_v8_cfocta --max_steps 8000

# CF-FA 训练（双路Scribble+Tile模式）
python train_controlnet_sd15_v8.py --mode cf2fa --name sd15_v8_cffa --max_steps 8000 --scribble_scale 0.8
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    """
    运行推理测试（每1000步）- Dual ControlNet 版本 v8 (Scribble + Tile)
...
def main():
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v8】")
    print("="*70)
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            if global_step % 100 == 0:
                ...
                # 构建日志消息
                msg_parts = [
                    f"[SD15-v8] step {global_step}/{max_steps}",
                ...
                msg_parts.extend([
                    f"(last_t={t_val:3d})",
                    f"Scribble:{args.scribble_scale}",
    ...
    print(f"  模型: SD 1.5 + Dual ControlNet (Scribble + Tile, FP32)")
