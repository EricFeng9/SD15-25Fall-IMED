...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        if is_cfoct:
            ...
            # 2. 保存预测图（模型直接输出的 512×512）
            img.save(os.path.join(img_out_dir, "pred.png"))
        ...
        try:
            ...
            try:
                if is_cfoct:
                    # CF_OCT 模式：使用 512×512 的 4×4 棋盘
                    pred_np_512 = np.array(img)  # 生成图已是 512×512
            ...
            if is_cfoct:
                # CF_OCT 模式：图像已经是 512×512，不需要 resize
                target_np = np.array(target_img_registered)  # (512, 512, 3)
            else:
                # CF-FA 和 CF-OCTA 模式：Resize 到 512×512（用于评估指标计算）
