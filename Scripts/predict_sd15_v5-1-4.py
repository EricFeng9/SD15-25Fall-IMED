"""
SD 1.5 双路 ControlNet 推理脚本 (HED + Tile) - v5-1-4
...
【使用说明】
基础用法：
  python predict_sd15_v5-1-4.py --mode cf2octa --name 251012_2

完整参数示例：
  python predict_sd15_v5-1-4.py \\
    --mode cf2octa \\
    --name 251012_2 \\
    --step 6000 \\
    --savedir test_6k \\
...
2. 使用特定步数的权重：
   python predict_sd15_v5-1-4.py --mode cf2octa --name 251012_2 --step 6000

3. 使用自定义保存目录：
   python predict_sd15_v5-1-4.py --mode cf2octa --name 251012_2 --step 6000 --savedir test_results
...
6. 反向任务（OCTA→CF）：
   python predict_sd15_v5-1-4.py --mode octa2cf --name 251012_octa2cf

【输出】
- 生成图像保存在：out_preds_sd15_dual/{mode}/{name}/[step_{N}|savedir]/
- 推理日志保存在：同目录下的 log.txt
- 第一张图像的调试输出：
  * cf2octa: 原图、绿色通道、取反图、HED边缘图、Tile输入
  * octa2cf: 原图、预处理图、HED边缘图、Tile输入（不取反）
"""
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + 双路 ControlNet 推理参数 (v5-1-4 绿色通道+取反版)\n")
    f.write("="*70 + "\n")
