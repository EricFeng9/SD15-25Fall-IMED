"""
SD 1.5 双路 ControlNet 推理脚本 (HED + Tile) - v6

【v6 更新】基于 v5-1-7，增加 CF-FA 数据集支持
- 支持 4 种推理模式：
  * cf2octa / octa2cf: 使用 CF-OCTA 数据集
  * cf2fa / fa2cf: 使用 CF-FA 数据集（新增）
- CF-FA 模式推理输出增强：
  * 原尺寸原图 (720×576)
  * 512×512 HED 图
  * 512×512 推理结果
...
- 【关键修复】根据模式区分预处理：
  * cf2octa/cf2fa: CF彩色图 → 绿色通道 + 取反 + HED（仅HED输入）
  * octa2cf/fa2cf: 直接使用原图 + HED（不取反）
...
基础用法：
  # CF-OCTA 推理
  python predict_sd15_v6.py --mode cf2octa --name 251012_2
  
  # CF-FA 推理
  python predict_sd15_v6.py --mode cf2fa --name 251017_3
...
2. 使用特定步数的权重：
   python predict_sd15_v6.py --mode cf2fa --name 251017_3 --step 7000

3. 使用自定义保存目录：
   python predict_sd15_v6.py --mode cf2fa --name 251017_3 --step 7000 --savedir test_results
...
6. 反向任务（FA→CF / OCTA→CF）：
   python predict_sd15_v6.py --mode fa2cf --name 251017_4 --step 8000
   python predict_sd15_v6.py --mode octa2cf --name 251013_1 --step 8000
...
- 第一张图像的调试输出：
  ...
  * CF-FA 模式（增强输出）：
    ...
    - {idx}_02_hed_512x512.png - 512×512 HED图
    - {idx}_03_pred_512x512_step{N}.png - 512×512 推理结果
    - {idx}_04_pred_{width}x{height}_step{N}.png - 原尺寸推理结果（720×576）
    - {idx}_05_target_registered_{width}x{height}.png - 配准后的原尺寸目标图
- 评估指标结果：
...
# ============ 参数解析 ============
parser = argparse.ArgumentParser(description="SD 1.5 + 双路 ControlNet 推理脚本 v6 (支持 CF-OCTA 和 CF-FA)")
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + 双路 ControlNet 推理参数 (v6 支持 CF-FA)\n")
    f.write("="*70 + "\n")
    ...
    if args.mode in ["cf2octa", "cf2fa"]:
        f.write("【预处理】CF模式: HED输入=绿色通道+取反 | Tile输入=原图\n")
        if is_cffa:
            f.write(f"【CF-FA】原始尺寸: {CFFA_ORIGINAL_SIZE[0]}×{CFFA_ORIGINAL_SIZE[1]}\n")
    else:
        f.write("【预处理】OCTA/FA模式: HED输入=原图 | Tile输入=原图\n")
    f.write("="*70 + "\n\n")
...
if args.mode == "cf2octa":
    ...
    print(f"  【配准修复】OCTA图保持原始尺寸（400×400），只做空间对齐")
else:
...
if args.mode in ["cf2octa", "cf2fa"]:
    print(f"  【预处理】CF模式: HED输入=绿色通道+取反 | Tile输入=原图")
    if is_cffa:
        print(f"  【CF-FA】原始尺寸: {CFFA_ORIGINAL_SIZE[0]}×{CFFA_ORIGINAL_SIZE[1]}，推理后resize回原尺寸")
else:
    print(f"  【预处理】OCTA/FA模式: HED输入=原图 | Tile输入=原图\n")
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        # 根据模式选择不同的预处理策略
        # HED 输入：CF 需要绿色通道+取反，其他直接使用
        # Tile 输入：始终使用原始彩色图（保留颜色和细节信息）
        if args.mode in ["cf2octa", "cf2fa"]:
            # CF→OCTA 或 CF→FA: 提取绿色通道 + 取反（仅用于 HED 边缘检测）
            ...
            src_img_for_hed = Image.fromarray(green_inverted).convert("RGB")
            src_img_for_tile = src_img_original  # Tile 使用原始彩色图
            
        else:  # octa2cf 或 fa2cf
            # OCTA→CF 或 FA→CF: 直接使用原图
            src_img_for_hed = src_img_original
            src_img_for_tile = src_img_original
            src_img_green = None  # 不需要绿色通道
        
        # 在预处理后的图像上做 HED 边缘检测（原始尺寸）
        cond_hed_original = hed_detector(src_img_for_hed)
        
        # 然后 resize 到 512×512
        cond_hed = cond_hed_original.resize((SIZE, SIZE))
        cond_tile = src_img_for_tile.resize((SIZE, SIZE))
        ...
        if processed_count == 0:
            ...
            if is_cffa:
                ...
                src_img_for_tile.resize((SIZE, SIZE)).save(os.path.join(out_dir, f"{idx}_01_input_512x512.png"))
                # 02: 512×512 HED图
                cond_hed.save(os.path.join(out_dir, f"{idx}_02_hed_512x512.png"))
                
                print(f"{'='*70}")
                print(f"✓ 已保存第一张图像的调试输出 (CF-FA 模式):")
                print(f"  模式: {args.mode}")
                print(f"  数据集: CF-FA")
                ...
                print(f"  02: {idx}_02_hed_512x512.png - 512×512 HED图")
                if args.mode == "cf2fa":
                    print(f"  预处理策略: HED输入=绿色通道+取反 | Tile输入=原图")
                else:
                    print(f"  预处理策略: HED输入=原图 | Tile输入=原图")
                ...
                src_img_for_hed.save(debug_processed_raw_path)
                ...
                print(f"{'='*70}")
                print(f"✓ 已保存第一张图像的调试输出 (CF-OCTA 模式):")
                print(f"  模式: {args.mode}")
                print(f"  数据集: CF-OCTA")
        ...
        if is_cffa:
            ...
            # 03: 512×512 推理结果
            img.save(os.path.join(out_dir, f"{idx}_03_pred_512x512_step{args.step if args.step else 'final'}.png"))
            
            # 04: Resize 回原尺寸的推理结果（720×576）
            pred_img_resized = img.resize(original_size)
            pred_img_resized.save(os.path.join(out_dir, f"{idx}_04_pred_{original_size[0]}x{original_size[1]}_step{args.step if args.step else 'final'}.png"))
        else:
            # CF-OCTA 模式：保持原有逻辑
        ...
        try:
            ...
            if is_cffa:
                # CF-FA 模式：05 保存配准后的原尺寸目标图
                target_img_registered.save(os.path.join(out_dir, f"{idx}_05_target_registered_{original_size[0]}x{original_size[1]}.png"))
            else:
                # CF-OCTA 模式：保持原有逻辑
            ...
            if processed_count == 0:
                if is_cffa:
                    # CF-FA 模式：简化的调试信息
                    print(f"\n  {'='*68}")
                    print(f"  ✓ 配准处理调试信息 (第一张图 - CF-FA 模式)")
                    print(f"  {'='*68}")
                    ...
                    print(f"  已保存配准后的目标图: 05_target_registered_{original_size[0]}x{original_size[1]}.png")
                    print(f"  {'='*68}\n")
                else:
                    # CF-OCTA 模式：保持原有逻辑
                    # 额外保存目标图的原始版本（用于对比）
                    ...
                    print(f"\n  {'='*68}")
                    print(f"  ✓ 配准处理调试信息 (第一张图 - CF-OCTA 模式)")
                    print(f"  {'='*68}")
                    print(f"  源图路径: {src_path}")
                    print(f"  源图尺寸: {src_img_original.size}")
                    print(f"  目标图路径: {target_path}")
                    ...
                    if args.mode == "octa2cf":
                        ...
                        if 'octa_size' in locals():
                            print(f"  ▶ 【v5-1-6修复】OCTA配准保持原始尺寸: {octa_size}")
                    print(f"  配准后尺寸: {target_img_registered.size}")
...
print(f"  每张图像输出:")
if is_cffa:
    print(f"    - {{idx}}_03_pred_512x512_step{{N}}.png (512×512 生成图像)")
    print(f"    - {{idx}}_04_pred_{{width}}x{{height}}_step{{N}}.png (原尺寸生成图像)")
    print(f"    - {{idx}}_05_target_registered_{{width}}x{{height}}.png (配准后目标图)")
else:
...
if len(valid_metrics) > 0:
    ...
    with open(log_path, "a") as f:
        ...
        if args.mode == "cf2octa":
            f.write("    【v5-1-6修复】OCTA配准保持原始尺寸，只做空间对齐，不改变尺度\n")
        f.write("="*70 + "\n")
