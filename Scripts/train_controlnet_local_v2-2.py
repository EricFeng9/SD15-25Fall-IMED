...
class PairCSV(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 加载条件图（不需要配准）
        cond = pil_to_tensor_rgb(Image.open(cond_path))
        ...
        if need_register and affine_path and os.path.exists(affine_path):
            ...
            tgt_pil = apply_affine_registration(tgt_pil, affine_matrix, (SIZE, SIZE))
...
def main():
    ...
    parser.add_argument("-n", "--name", "-name", dest="name", default='v2-2_cf_input')
    ...
    print(f"[v2] 配准逻辑: cf2octa 使用 OCTA→CF 矩阵, octa2cf 使用 CF→OCTA 矩阵")
    ...
    while global_step < max_steps:
        for cond, tgt in train_loader:
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(pipe.controlnet.parameters(), max_norm=1.0)  # 梯度裁剪
            opt.step()
            ...
            if global_step % 100 == 0:
                ...
                msg = f"[v2] step {global_step}/{max_steps} loss {loss.item():.4f} | 100step {elapsed:.2f}s ({elapsed/100:.3f}s/step)"
                print(msg)
