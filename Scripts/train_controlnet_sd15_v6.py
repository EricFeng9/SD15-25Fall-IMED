# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + 双路 ControlNet 版本 v6

【基于】train_controlnet_sd15_v5-3.py
...
【v6 更新】
1. 模块化数据加载
   - CF-OCTA: 使用 data_loader_cfocta.py
   - CF-FA: 使用 data_loader_cffa.py

2. 支持 4 种训练模式
   - cf2octa: CF → OCTA (使用 CF-OCTA 数据集)
   - octa2cf: OCTA → CF (使用 CF-OCTA 数据集)
   - cf2fa: CF → FA (使用 CF-FA 数据集) ⭐新增
   - fa2cf: FA → CF (使用 CF-FA 数据集) ⭐新增
...
【使用方法】
# CF-OCTA 训练
python train_controlnet_sd15_v6.py --mode cf2octa --name sd15_v6_cfocta --max_steps 8000

# CF-FA 训练
python train_controlnet_sd15_v6.py --mode cf2fa --name sd15_v6_cffa --max_steps 8000
...
# 导入两个数据加载器
from data_loader_cfocta import PairCSV as PairCSV_CFOCTA, SIZE, get_hed_detector
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    """
    运行推理测试（每1000步）- 双路 ControlNet 版本 v6
    ...
    """
    print(f"\n{'='*70}")
    print(f"运行推理测试 (step {step_num}) - 双路 ControlNet [{mode}]")
    print(f"{'='*70}")
    ...
    cond_hed_original = hed(src_img_for_hed)
    
    # 4. Resize 到 512×512
    cond_hed_pil = cond_hed_original.resize((SIZE, SIZE))
    cond_tile_pil = src_img_for_tile.resize((SIZE, SIZE))
    ...
    if is_cffa:
        ...
        # 2. 512×512 原图（Tile输入）
        src_img_for_tile.resize((SIZE, SIZE)).save(os.path.join(infer_dir, f"{idx}_01_input_512x512.png"))
        # 3. 512×512 HED图
        cond_hed_pil.save(os.path.join(infer_dir, f"{idx}_02_hed_512x512.png"))
    else:
        # CF-OCTA 模式：保持原有逻辑
        ...
        src_img_for_hed.save(os.path.join(infer_dir, f"{idx}_input_processed.png"))
    ...
    if is_cffa:
        ...
        # 5. Resize 回原尺寸的推理结果（720×576）
        pred_img_resized = pred_img.resize(original_size)  # resize 回原尺寸
        pred_img_resized.save(os.path.join(infer_dir, f"{idx}_04_pred_{original_size[0]}x{original_size[1]}_step{step_num}.png"))
    else:
        # CF-OCTA 模式：保持原有逻辑
    ...
    if is_cffa:
        ...
        try:
            ...
            if cf_pts_path and fa_pts_path and os.path.exists(cf_pts_path) and os.path.exists(fa_pts_path):
                ...
                # 6. 保存配准后的原尺寸目标图
                target_img_registered.save(os.path.join(infer_dir, f"{idx}_05_target_registered_{original_size[0]}x{original_size[1]}.png"))
...
def main():
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + 双路 ControlNet 训练配置 v6】")
    print("="*70)
