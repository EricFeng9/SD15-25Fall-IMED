"""
SD 1.5 双路 ControlNet 推理脚本 (HED + Tile) - v5-1-2
...
【使用说明】
基础用法：
  python predict_sd15_v5-1-2.py --mode cf2octa --name 251012_2

完整参数示例：
  python predict_sd15_v5-1-2.py \\
    --mode cf2octa \\
    --name 251012_2 \\
    --step 6000 \\
    --savedir test_6k \\
...
2. 使用特定步数的权重：
   python predict_sd15_v5-1-2.py --mode cf2octa --name 251012_2 --step 6000

3. 使用自定义保存目录：
   python predict_sd15_v5-1-2.py --mode cf2octa --name 251012_2 --step 6000 --savedir test_results
...
6. 反向任务（OCTA→CF）：
   python predict_sd15_v5-1-2.py --mode octa2cf --name 251012_octa2cf
...
# ============ 参数解析 ============
parser = argparse.ArgumentParser(description="SD 1.5 + 双路 ControlNet 推理脚本 (修复版)")
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + 双路 ControlNet 推理参数 (v5-1-2 修复版)\n")
    f.write("="*70 + "\n")
...
print("\n" + "="*70)
print("正在加载 SD 1.5 + 双路 ControlNet 模型 (修复版)...")
print("="*70)
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        # 【修复】在原图尺寸上做 HED 边缘检测（与训练一致）
        cond_hed_original = hed_detector(src_img_original)
        ...
        if processed_count == 0:
            ...
            # 1. 保存原始图像（未 resize 的）
            debug_original_raw_path = os.path.join(out_dir, f"{idx}_input_original_raw.png")
            src_img_original.save(debug_original_raw_path)
            
            # 2. 保存 resize 后的原图（512×512）
            debug_original_path = os.path.join(out_dir, f"{idx}_input_original_512.png")
            cond_tile.save(debug_original_path)
            
            # 3. 保存原始尺寸的 HED 边缘图
            debug_hed_raw_path = os.path.join(out_dir, f"{idx}_input_hed_raw.png")
            cond_hed_original.save(debug_hed_raw_path)
            
            # 4. 保存 resize 后的 HED 边缘图（ControlNet 实际输入）
            debug_hed_path = os.path.join(out_dir, f"{idx}_input_hed_512.png")
            cond_hed.save(debug_hed_path)
            
            # 5. 保存 Tile 输入（与 resize 后原图相同）
            ...
            print(f"  1. {debug_original_raw_path} - 原始图像（原尺寸）")
            print(f"  2. {debug_hed_raw_path} - HED边缘图（原尺寸）")
            print(f"  3. {debug_original_path} - 原图 resize 到 512×512")
            print(f"  4. {debug_hed_path} - HED边缘图 resize 到 512×512 (ControlNet-HED输入)")
            print(f"  5. {debug_tile_path} - Tile输入 512×512 (ControlNet-Tile输入)")
            print(f"  源路径: {src_path}")
            print(f"  【关键修复】HED 检测在原图尺寸进行（与训练一致）")
            print(f"{'='*70}\n")
        
        # 【修复3】每张图都创建新的 generator（保证一致性）
        generator = torch.Generator(device=device).manual_seed(used_seed)
        
        # 【修复4】使用 torch.no_grad()（节省显存，与训练脚本一致）
        with torch.no_grad():
            # SD 1.5 双路推理
            img = pipe(
                prompt=args.prompt,
