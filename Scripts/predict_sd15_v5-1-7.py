"""
SD 1.5 双路 ControlNet 推理脚本 (HED + Tile) - v5-1-6
...
- 【v5-1-6 关键修复】cf2octa 配准逻辑修复：
  * OCTA图配准时保持原始尺寸（400×400），不改变尺度
  * 配准只做空间对齐，output_size = OCTA原始尺寸
  * 修复了之前错误地配准到CF尺寸（256×256）导致的OCTA图被压缩问题

【使用说明】
基础用法：
  python predict_sd15_v5-1-6.py --mode cf2octa --name 251012_2

完整参数示例：
  python predict_sd15_v5-1-6.py \\
    --mode cf2octa \\
    --name 251012_2 \\
    --step 6000 \\
    --savedir test_6k \\
...
2. 使用特定步数的权重：
   python predict_sd15_v5-1-6.py --mode cf2octa --name 251012_2 --step 6000

3. 使用自定义保存目录：
   python predict_sd15_v5-1-6.py --mode cf2octa --name 251012_2 --step 6000 --savedir test_results
...
6. 反向任务（OCTA→CF，自动评估）：
   python predict_sd15_v5-1-6.py --mode octa2cf --name 251013_1 --step 8000
...
- 每张图像的输出：
  * {idx}_pred_{octa/cf}.png - 生成的预测图像
  * {idx}_target_registered.png - 配准后的目标图（送入评估网络的实际目标图）
- 评估指标结果：
  ...
  * cf2octa 模式的评估使用正确配准的 OCTA 目标图（保持原始尺寸，v5-1-6修复）
"""
...
from registration_cf_octa import load_affine_matrix, apply_affine_registration  # 配准工具模块
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + 双路 ControlNet 推理参数 (v5-1-6 智能预处理+自动评估+cf2octa配准修复)\n")
    f.write("="*70 + "\n")
    ...
    if args.mode == "cf2octa":
        ...
        f.write("【配准修复】OCTA图保持原始尺寸（400×400），只做空间对齐\n")
    else:
