# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v9-2

【基于】train_controlnet_sd15_v9-1.py
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    ...
    # 2. 根据数据集类型选择不同的预处理策略
    if is_cffa:
        # CF-FA模式：与训练一致 - 先从原图提取血管，再 resize
        ...
        # CF_OCT模式：【v9-2 新配准方案】先 resize_with_padding，再提取血管
        from registration_cf_oct import resize_with_padding
        from data_loader_cfoct_v2 import extract_vessel_hessian
        
        # 先 resize_with_padding 条件图
        ...
        # CF-OCTA模式：与训练一致 - 先从原图提取血管，再 resize
        ...
        cond_scribble_pil = extract_vessel_frangi_cfocta(
            ...
            image_type=image_type,
            sigmas=FRANGI_SIGMAS,
            gamma_cf=GAMMA_CFOCTA_CF,
            gamma_octa=GAMMA_CFOCTA_OCTA
        )
        # Resize Scribble条件图
        cond_scribble_pil = cond_scribble_pil.resize((SIZE, SIZE))
        # Tile 输入：直接 resize 原图
...
def main():
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v9-2】✨")
    print("="*70)
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            if global_step % 100 == 0:
                ...
                # 构建日志消息
                msg_parts = [
                    f"[SD15-v9-2] step {global_step}/{max_steps}",
                    f"lr:{current_lr:.2e}",
                    f"mse:{avg_mse:.4f}",
                ]
    ...
    print(f"  模型版本: SD 1.5 + Dual ControlNet v9-2 (Scribble + Tile, FP32)")
    print(f"  数据集类型: {'CF-FA' if is_cffa else ('CF_OCT' if is_cfoct else 'CF-OCTA')}")
