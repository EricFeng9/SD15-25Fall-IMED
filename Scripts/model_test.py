...
- 然后从对应尺寸提取血管（gamma统一0.015），再resize到512×512推理
- 确保每个模型看到的输入分布与训练时一致
...
# ============ Frangi 血管滤波参数（统一）============
GAMMA_CF_UNIFIED = 0.015  # v4: 所有CF图统一使用0.015
FRANGI_SIGMAS = range(1, 16)
...
with open(log_path, "w") as f:
    ...
    f.write(f"  gamma_cf统一: {GAMMA_CF_UNIFIED}\n")
    f.write(f"  frangi_sigmas: range(1, 16)\n")
...
for i, img_path in enumerate(image_files):
    try:
        ...
        for mode in modes_to_run:
            ...
            # Step 3: 从512×512图提取血管（gamma统一0.015）
            cond_scribble = extract_vessel_hessian(
                cf_512_pil,
                image_type='cf',
                gamma=GAMMA_CF_UNIFIED,
        ...
        with open(log_path, "a") as f:
            ...
            f.write(f"    正向: 输入CF图({original_width}×{original_height}) → resize_with_padding到训练集尺寸 → resize到512×512 → 提取血管(gamma={GAMMA_CF_UNIFIED}) → 推理\n")
...
with open(log_path, "a") as f:
    ...
    f.write(f"统一gamma_cf={GAMMA_CF_UNIFIED}\n")
    f.write("="*70 + "\n")
