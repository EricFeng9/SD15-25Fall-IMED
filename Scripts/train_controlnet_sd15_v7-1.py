# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Tile ControlNet 版本 v7-1
...
def focal_frequency_loss(pred, target, alpha=1.0):
    ...
    # Focal 权重（自适应：误差大的频率获得更大权重）
    weight = torch.abs(spectrum_diff) ** alpha
    
    # 加权损失
    loss = weight * (spectrum_diff ** 2)
...
def main():
    ...
    # Focal Frequency Loss 参数
    parser.add_argument("--ffl_lambda", type=float, default=0.01,
                        help="Focal Frequency Loss 权重 (设为0则禁用，推荐0.05-0.2)")
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Tile ControlNet 训练配置 v7-1】")
    print("="*70)
    ...
    if args.ffl_lambda > 0:
        loss_components.append(f"FFL (λ={args.ffl_lambda}, α={args.ffl_alpha})")
        ...
        for batch_data in train_loader:
            ...
            if args.msssim_lambda > 0 or args.ffl_lambda > 0:
                ...
                if args.ffl_lambda > 0:
                    ...
                    loss_ffl = focal_frequency_loss(pred_imgs_ffl, tgt_imgs_ffl, alpha=args.ffl_alpha)
                ...
                if args.ffl_lambda > 0:
                    loss = loss + args.ffl_lambda * loss_ffl
            else:
                loss = loss_mse
            ...
            if global_step % 100 == 0:
                ...
                # 构建日志消息
                msg_parts = [
                    f"[SD15-v7-1] step {global_step}/{max_steps}",
                ...
                if args.ffl_lambda > 0 and len(ffl_loss_accumulator) > 0:
                    ...
                    msg_parts.append(f"ffl: {avg_ffl:.4f}")
                    ffl_loss_accumulator = []
