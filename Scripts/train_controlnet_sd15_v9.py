# -*- coding: utf-8 -*-
'''
训练脚本 - SD 1.5 + Dual ControlNet 版本 v8-3-3

【基于】train_controlnet_sd15_v8-3-1.py
...
2. **血管结构损失 (Vessel Loss) - 所有模式启用**
   ...
   - CF-FA数据集：sigmas=range(1,16), gamma_cffa=0.015
     * CF图像：提取绿色通道 + 取反（暗血管→亮血管）
     * FA图像：提取绿色通道（亮血管）
   - CF-OCTA数据集：sigmas=range(1,16)
     * CF图像：gamma=0.008（Scribble用绿色通道+取反提取血管）
     * OCTA图像：gamma=0.1（直接转灰度）
...
python train_controlnet_sd15_v8-3-2.py --mode cf2fa --name sd15_v8-3-2_cffa --max_steps 8000 --scribble_scale 0.8 --vessel_lambda 0.05

# CF-OCTA 训练（双路Vessel+Tile模式，gamma_cf=0.008, gamma_octa=0.1）
python train_controlnet_sd15_v8-3-2.py --mode cf2octa --name sd15_v8-3-2_cfocta --max_steps 8000 --scribble_scale 0.8 --vessel_lambda 0.05
...
参数调节:
  ...
  --vessel_lambda: Vessel Loss 权重 (默认 0.05，所有模式生效)
'''
...
# 导入三个数据加载器
from data_loader_cfocta_v2 import PairCSV as PairCSV_CFOCTA, SIZE  # v2: 使用Vessel血管滤波
from data_loader_cffa_v2 import PairCSV_CFFA, SIZE as SIZE_CFFA
from data_loader_cfoct_v2 import PairCSV_CFOCT, SIZE as SIZE_CFOCT  # CF_OCT 数据集
...
def compute_vessel_loss_frangi(pred_imgs, gt_imgs, mode='cf2fa', sigmas=range(1, 16), 
                                alpha=0.5, beta=0.5, gamma_cffa=0.015, gamma_cfocta_cf=0.008, gamma_cfocta_octa=0.1,
                                gamma_cfoct_cf=0.015, gamma_oct=0.05,
                                debug_dir=None):
    """
    ...
    参数:
        ...
        sigmas: Frangi 多尺度参数（默认 range(1, 16)）
        alpha: Frangi 板状结构敏感度（默认 0.5）
        beta: Frangi 球状结构敏感度（默认 0.5）
        gamma_cffa: CF-FA模式的gamma值（默认 0.015）
        gamma_cfocta_cf: CF-OCTA模式的CF图gamma值（原gamma_cf，默认 0.008）
        gamma_cfocta_octa: CF-OCTA模式的OCTA图gamma值（原gamma_octa，默认 0.1）
        gamma_cfoct_cf: CF_OCT模式的CF图gamma值（默认 0.015）
        gamma_oct: CF_OCT模式的OCT图gamma值（默认 0.05）
    ...
    if debug_dir is not None:
        ...
        print(f"  - vessel_loss_pred_frangi.png: 预测图Frangi响应")
        print(f"  - vessel_loss_gt_frangi.png: GT图Frangi响应\n")
    
    # 6. 计算 L1 距离（只在有效区域）
    diff = torch.abs(pred_vessel - gt_vessel) * valid_mask.float()
    loss = diff.sum() / (valid_mask.sum() + 1e-10)
...
def run_inference_test(row_data, step_dir, step_num, mode, fixed_seed=42):
    ...
    if is_cffa:
        ...
        # CF_OCT模式：使用Hessian血管提取（CF和OCT血管都是暗色）
        from data_loader_cfoct_v2 import extract_vessel_hessian
        # cf2oct: CF图（血管暗色）
        # oct2cf: OCT图（血管暗色）
        image_type = 'cf' if mode == "cf2oct" else 'oct'
        gamma_value = 0.015 if image_type == 'cf' else 0.1
        cond_scribble_pil = extract_vessel_hessian(
            src_img_original,
            image_type=image_type,
            gamma=gamma_value,
        ...
        # CF-OCTA模式：使用Frangi滤波提取血管
        from data_loader_cfocta_v2 import extract_vessel_frangi_cfocta
        
        # 判断图像类型（extract_vessel_frangi_cfocta内部会处理预处理）
        if mode == "cf2octa":
            # CF图：直接传入原始彩色图（函数内部会提取绿色通道+取反）
            image_type = 'cf'
        else:  # octa2cf
            # OCTA图：直接传入原始图
            image_type = 'octa'
        
        # 应用Frangi滤波（函数内部会根据image_type自动处理）
        cond_scribble_pil = extract_vessel_frangi_cfocta(
            src_img_original,  # 直接传入原始图
            image_type=image_type,
            sigmas=range(1, 16),
    ...
    # 3. Tile ControlNet 输入图像
    # 所有模式都直接使用原始彩色图（保留颜色和细节信息）
    ...
    if is_cffa:
        ...
        try:
            ...
            if cf_pts_path and oct_pts_path and os.path.exists(cf_pts_path) and os.path.exists(oct_pts_path):
                ...
                # 加载配对点
                cond_points = load_keypoints(cf_pts_path if mode == "cf2oct" else oct_pts_path)
                tgt_points = load_keypoints(oct_pts_path if mode == "cf2oct" else cf_pts_path)
                
                # 获取原始图像尺寸
                src_img_original = Image.open(src_path).convert("RGB")
                cond_w, cond_h = src_img_original.size
                tgt_w, tgt_h = target_img_original.size
                
                # 【归一化方法】计算仿射矩阵（在512×512空间）
                affine_matrix = compute_affine_from_normalized_points(
                    tgt_points, cond_points,
                    src_img_size=(tgt_w, tgt_h),  # 目标图尺寸
                    dst_img_size=(cond_w, cond_h),  # 条件图尺寸
                    target_size=(SIZE, SIZE)
                )
                
                # 先resize到512×512，再应用配准
                target_img_512 = target_img_original.resize((SIZE, SIZE))
                target_np = np.array(target_img_512)
                registered_np = apply_affine_cfoct(
                    target_np, affine_matrix, 
...
def main():
    ...
    if is_cffa:
        ...
        # 使用 CF_OCT 数据加载器
        train_ds = PairCSV_CFOCT(args.train_csv, args.mode)
        val_ds = PairCSV_CFOCT(args.val_csv, args.mode)
    else:
        # 使用 CF-OCTA 数据加载器
    ...
    # ============ 训练信息打印 ============
    print("\n" + "="*70)
    print("【SD 1.5 + Dual ControlNet 训练配置 v8-3-3】✨")
    print("="*70)
    ...
    print(f"  │   Vessel (Frangi+L1): {args.vessel_lambda} (固定)")
    ...
    while global_step < max_steps:
        for batch_data in train_loader:
            ...
            # 4. 【v9 更新】血管结构损失 (Frangi + L1) - 支持 CF_OCT
            ...
            loss_vessel = compute_vessel_loss_frangi(
                ...
                alpha=0.5, 
                beta=0.5, 
                gamma_cffa=0.015,           # CF-FA模式使用
                gamma_cfocta_cf=0.008,      # CF-OCTA模式，CF图使用
                gamma_cfocta_octa=0.1,      # CF-OCTA模式，OCTA图使用
                gamma_cfoct_cf=0.015,       # CF_OCT模式，CF图使用
                gamma_oct=0.05,              # CF_OCT模式，OCT图使用
            ...
            if global_step % 100 == 0:
                ...
                # 构建日志消息
                msg_parts = [
                    f"[SD15-v8-3-3] step {global_step}/{max_steps}",
                    f"lr:{current_lr:.2e}",
                    f"mse:{avg_mse:.4f}",
                ]
    ...
    print(f"  模型版本: SD 1.5 + Dual ControlNet v8-3-3 (Scribble + Tile, FP32)")
    print(f"  数据集类型: {'CF-FA' if is_cffa else 'CF-OCTA'}")
    if not is_cffa:
        print(f"  v8-3-3 更新: ")
        print(f"    1. 修复 octa2cf 模式 Vessel Loss 逻辑（CF图正确取反）")
        print(f"    2. 修复配准黑边干扰（取反前先将黑边替换成白色）")
        print(f"  v8-3-2 更新: CF训练集改为彩色原图，Tile输入直接使用原图")
    else:
        print(f"  v8-3-3 更新: 修复配准黑边干扰（CF图取反前先将黑边替换成白色）")
    print(f"  ControlNet 强度: Scribble={args.scribble_scale}, Tile={args.tile_scale}")
    print(f"  损失函数: MSE + MS-SSIM(λ={args.msssim_lambda}) + Vessel-Frangi(λ={args.vessel_lambda})")
    if is_cffa:
