...
4. 继承 v14 逻辑：保留 MSE + MS-SSIM + Vessel Dice + LPIPS Loss 组合。
...
class VGGPerceptualLoss(torch.nn.Module):
    def __init__(self, resize=True):
        ...
        blocks = []
        # 加载 VGG16 预训练权重
        blocks.append(models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_V1).features[:4].eval())
        blocks.append(models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_V1).features[4:9].eval())
        blocks.append(models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_V1).features[9:16].eval())
        blocks.append(models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_V1).features[16:23].eval())
        for bl in blocks:
...
def compute_total_loss(noise_pred, noise, noisy_latents, latents, timesteps, vae, noise_scheduler, msssim_fn, lpips_fn, args):
    """计算综合损失：MSE + MS-SSIM + Vessel Dice + LPIPS"""
    # 1. 噪声空间 MSE 损失
    ...
    # 4. LPIPS 感知损失
    loss_lpips = lpips_fn(pred_imgs, gt_imgs) if args.lpips_lambda > 0 else torch.tensor(0.0).to(DEVICE)
    
    # 组合总损失
    total_loss = loss_mse + args.msssim_lambda * loss_msssim + args.vessel_lambda * loss_vessel + args.lpips_lambda * loss_lpips
    return total_loss, loss_mse, loss_msssim, loss_vessel, loss_lpips
...
def main():
    ...
    parser.add_argument("--lpips_lambda", type=float, default=0.5)
    ...
    lpips_fn = VGGPerceptualLoss().to(DEVICE)
    ...
    while global_step < args.max_steps:
        for batch in train_loader:
            ...
            # 计算 Loss
            loss, l_mse, l_ssim, l_vessel, l_lpips = compute_total_loss(noise_pred, noise, noisy_latents, latents, timesteps, vae, noise_scheduler, msssim_fn, lpips_fn, args)
            ...
            lpips_loss_accumulator.append(l_lpips.item())
            ...
            if global_step % 100 == 0:
                ...
                avg_lpips = np.mean(lpips_loss_accumulator)
                
                # 计算加权后的值用于打印
                w_ssim = avg_ssim * args.msssim_lambda
                w_vessel = avg_vessel * args.vessel_lambda
                w_lpips = avg_lpips * args.lpips_lambda
                w_total = avg_mse + w_ssim + w_vessel + w_lpips
                ...
                if args.lpips_lambda > 0:
                    msg_parts.append(f"lpips:{w_lpips:.4f}(λ={args.lpips_lambda})")
