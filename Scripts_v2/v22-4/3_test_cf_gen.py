# -*- coding: utf-8 -*-
"""
CF 生成模型推理脚本 (V22-4-CF-GEN-TEST)
--------------------------------------

【v22-4 改进】
1. 动态 CFG：前期高 CFG 确定结构，后期低 CFG 保留纹理
2. 默认 CFG Scale 降至 3.5
3. 可选后处理传感器噪声注入，恢复真实眼底图颗粒感

使用方式（示例）：
CUDA_VISIBLE_DEVICES=0 python Scripts_v2/v22-4/3_test_cf_gen.py \\
  -n cf_gen_v22_4 \\
  --amount 1000 \\
  --savedir 1 \\
  --add_sensor_noise
...
CAPTION_FILE = os.path.join(os.path.dirname(__file__), "cf_captions.json")
try:
...
def add_realistic_fundus_noise(img_pil, noise_level=0.02):
    """
    【v22-4 新增】为生成的 CF 添加模拟真实眼底相机的传感器噪声。
    用于后处理，恢复真实眼底图的颗粒感，避免生成图过于平滑。
    """
...
@torch.no_grad()
def generate_cf_images(unet, vae, tokenizer, text_encoder, uncond_embeds,
                       amount: int, out_dir: str, steps: int = 50, guidance_scale: float = 3.5,
    ...
    for idx in indices:
        ...
        # [调试用] 前3张图打印 prompt
        if idx <= 3:
            print(f"\n[DEBUG 生成图片 {idx}] 随机 Prompt:")
            print(f"  {current_prompt_str[:80]}...")
        ...
        if add_sensor_noise:
            ...
            img_cf = add_realistic_fundus_noise(img_cf, noise_level)
...
def generate_cf_from_real_cf(
    ...
    for cf_path in cf_iter:
        ...
        # [修改] 查出这张原图的专属 Prompt
        key = get_caption_key_from_path(cf_path)
        current_prompt_str = CF_CAPTIONS.get(key, random.choice(ALL_PROMPTS_LIST))
        prompt_cf = encode_dynamic_prompts([current_prompt_str], tokenizer, text_encoder)
        
        # [调试用] 前3张图打印匹配信息
        if debug_counter < 3:
            is_matched = key in CF_CAPTIONS
            status = "✓ 匹配成功" if is_matched else "✗ 使用随机 fallback"
            print(f"\n[DEBUG 图片 {debug_counter+1}] {status}")
            print(f"  路径: {cf_path}")
            print(f"  Key: {key}")
            print(f"  Prompt: {current_prompt_str[:80]}...")
            debug_counter += 1
        ...
        for _ in range(max(1, num_per_cf)):
            ...
            while retries < max_retries:
                ...
                if add_sensor_noise:
                    noise_level = random.uniform(0.01, 0.03)
                    img_cf_noisy = add_realistic_fundus_noise(img_cf, noise_level)
                    img_cf_noisy.save(os.path.join(img_dir, "cf.png"))  # 覆盖原图
...
def main():
    parser = argparse.ArgumentParser(description="V22-4 CF 生成模型推理脚本")
    parser.add_argument(
        "-n", "--name", default="cf_gen_v22_4",
    ...
    parser.add_argument(
        ...
        help="CFG scale（v22-4 默认 3.5，降低以减少伪影）",
    )
    parser.add_argument(
    ...
    print(f"  - CFG Scale: {args.guidance_scale}（动态 CFG 已启用）")
    ...
    # 预计算无条件 embedding（用于 CFG）
    print("\n[2/4] 预计算无条件 embedding（用于 CFG）...")
    uncond_embeds = encode_dynamic_prompts([""], tokenizer, text_encoder)
    print(f"  ✓ 无条件 embedding shape: {uncond_embeds.shape}")

    # 3. 加载训练好的 UNet LoRA
    #    - 默认：使用 best_checkpoint
    #    - 如果提供了 --ckpt_dir：优先使用该目录
    print("\n[3/4] 加载训练好的 UNet LoRA ...")
    if args.ckpt_dir:
        # 支持两种形式：
    ...
    if args.mode == "noise":
        ...
        print("\n[4/4] 开始从真实 CF 加噪声生成增强 CF 图像（使用 CFG + 血管占比过滤）...")
