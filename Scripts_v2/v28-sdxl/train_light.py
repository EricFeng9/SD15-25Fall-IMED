...
def main():
    ...
    # 重新划分数据集为 9:1 (原来是 8:2)
    from torch.utils.data import Subset
    
    full_train_ds = CFFADataset_v2(split="train", mode="cf2fa")
    full_val_ds = CFFADataset_v2(split="test", mode="cf2fa")
    ...
    # 按9:1划分
    new_train_size = int(total_samples * 0.9)
    new_val_size = total_samples - new_train_size
    
    # 使用原训练集的所有数据 + 部分验证集数据作为新训练集
    train_indices = list(range(total_train))
    val_indices_for_train = list(range(min(new_train_size - total_train, total_val)))
    
    # 剩余验证集数据作为新验证集
    val_indices = list(range(len(val_indices_for_train), total_val))
    
    train_ds = full_train_ds  # 使用完整训练集
    val_ds = Subset(full_val_ds, val_indices)  # 验证集缩小
    
    print(f"\n========== 数据集划分 (9:1) ==========")
    print(f"训练集: {len(train_ds)} 样本")
    print(f"验证集: {len(val_ds)} 样本 (从 {total_val} 减少到 {len(val_ds)})")

    # 增加num_workers和启用pin_memory加速数据加载
    ...
    print(f"\n========== 优化配置 ==========")
    print(f"✓ 可视化: 5张图像, 30步采样")
    print(f"✓ 可视化频率: 每1000步 + loss改善时")
    print(f"✓ 验证频率: 每500步")
    print(f"✓ 数据集划分: 9:1 (训练:验证)\n")
    ...
    while global_step < args.max_steps:
        for batch in train_loader:
            ...
            if global_step % 500 == 0:
                ...
                # 可视化策略：每1000步 或 loss改善时
                should_visualize = False
                vis_reason = ""
                
                if global_step % 1000 == 0:
                    should_visualize = True
                    vis_reason = "定期可视化 (每1000步)"
                ...
                if should_visualize:
                    ...
                    print(f"[可视化] {vis_reason} - 在 {vis_dir} 生成 5 组随机 CF-FA 图像 (30步采样)...")
                    visualize_random_pairs(
                        unet, vae, tokenizer, tokenizer_2, text_encoder, text_encoder_2, 
                        num_samples=5, out_dir=vis_dir, steps=30
                    )
