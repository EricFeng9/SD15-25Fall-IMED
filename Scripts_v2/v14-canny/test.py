...
ctrl_root = "/data/student/Fengjunming/SDXL_ControlNet/results/out_ctrl_sd15_canny"

# 预训练的 Canny ControlNet
ctrl_canny_pretrained = "/data/student/Fengjunming/SDXL_ControlNet/models/controlnet-sd15-canny"
...
out_root = "/data/student/Fengjunming/SDXL_ControlNet/results/out_preds_sd15_canny"

# CF-FA 原始图像尺寸
CFFA_ORIGINAL_SIZE = (720, 576)  # width, height

# ============ 参数解析 ============
parser = argparse.ArgumentParser(description="SD 1.5 + Canny ControlNet 推理脚本")
...
# Canny ControlNet 参数
parser.add_argument("--prompt", default="", help="文本提示词（正向）")
parser.add_argument("--negative_prompt", default="", help="文本提示词（负向）")
parser.add_argument("--canny_scale", type=float, default=1.0,
                    help="Canny ControlNet 条件强度 (0.0-2.0)")
...
# 检查 ControlNet 子目录（单路 Canny）
ctrl_canny_dir = os.path.join(ctrl_dir, "controlnet_canny")
if not os.path.isdir(ctrl_canny_dir):
    raise FileNotFoundError(
        f"未找到 Canny ControlNet 目录:\n"
        f"  Canny: {ctrl_canny_dir} - {'存在' if os.path.isdir(ctrl_canny_dir) else '缺失'}\n"
        f"请确认使用的是单路 Canny ControlNet 训练的模型"
    )
...
with open(log_path, "a") as f:
    f.write("="*70 + "\n")
    f.write("SD 1.5 + Canny ControlNet 推理参数\n")
    f.write("="*70 + "\n")
    ...
    f.write(f"canny_scale={args.canny_scale}\n")
...
print(f"  ControlNet-Canny: {ctrl_canny_dir}")
...
# 加载单路 Canny ControlNet
controlnet_canny = ControlNetModel.from_pretrained(
    ctrl_canny_dir,
    torch_dtype=dtype, 
    local_files_only=True
).to(device)
print("✓ Canny ControlNet 加载完成")
...
print(f"  输出: {out_dir}")
print(f"  参数: canny_scale={args.canny_scale}, cfg={args.cfg}, steps={args.steps}, seed={used_seed}\n")
...
with open(args.csv) as f:
    for i, row in enumerate(csv.DictReader(f)):
        ...
        try:
            ...
            cond_canny = results['canny_input']  # 512×512 Canny 输入
            cond_processed = results['cond_processed']  # 512×512 条件图
            ...
            # 基于条件输入生成黑边蒙版
            cond_np_for_mask = np.array(cond_processed)
            mask_512 = mask_gen(
                cond_np_for_mask,
                threshold=10,  # 固定阈值
                smooth=True,  # 平滑边缘
        ...
        # 3. 保存 Canny 输入与条件图
        cond_canny.save(os.path.join(img_out_dir, "canny_512x512.png"))
        cond_processed.save(os.path.join(img_out_dir, "cond_input_512x512.png"))
