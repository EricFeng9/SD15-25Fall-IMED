...
def calculate_brightness_ratio(pred, gt):
    ...
    计算 pred 和 gt 的亮度比值
    """
    pred_brightness = calculate_brightness(pred)
    gt_brightness = calculate_brightness(gt)
...
def test_double_masking_issue():
    ...
    # 计算亮度比值
    pred_brightness, gt_brightness, brightness_ratio = calculate_brightness_ratio(pred, gt)
    print(f"预测图像平均亮度: {pred_brightness:.2f}")
    print(f"目标图像平均亮度: {gt_brightness:.2f}")
    print(f"亮度比值 (pred/gt): {brightness_ratio:.4f}")
    ...
    if os.path.exists(result_dir):
        ...
        if samples:
            ...
            if os.path.exists(pred_path) and os.path.exists(target_path):
                ...
                # 计算亮度比值
                pred_brightness, gt_brightness, brightness_ratio = calculate_brightness_ratio(pred, target)
                print(f"预测图像平均亮度: {pred_brightness:.2f}")
                print(f"目标图像平均亮度: {gt_brightness:.2f}")
                print(f"亮度比值 (pred/gt): {brightness_ratio:.4f}")
