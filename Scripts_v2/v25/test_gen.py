# -*- coding: utf-8 -*-
"""
Joint CF-FA Generation Inference Script (v25)
---------------------------------------------

用途：
- 加载 v25/train.py 训练好的 Joint CF-FA 生成模型（UNet+LoRA），
  生成新的 CF-FA 成对图像。
...
输入（命令行参数）：
- name      : 训练实验名称（对应 out_joint_sd15_cffa_pairs 下的子目录）
- savedir   : 推理结果子目录名称
...
输出目录结构：
- /results/out_joint_sd15_cffa_pairs_preds/{name}/{savedir}/
  - 1/cf.png
  - 1/fa.png
  - 1/joint.png
...
TRAIN_OUT_ROOT = "/data/student/Fengjunming/SDXL_ControlNet/results/out_joint_sd15_cffa_pairs"
PRED_OUT_ROOT = "/data/student/Fengjunming/SDXL_ControlNet/results/out_joint_sd15_cffa_pairs_preds"
...
@torch.no_grad()
def generate_pairs_from_noise(unet, vae, tokenizer, text_encoder,
    ...
    prompt_joint = get_joint_prompt_embeds(1, tokenizer, text_encoder)
    ...
    if tqdm is not None:
        indices = tqdm(indices, desc="生成 Joint CF-FA 图像对", ncols=80)
    ...
    for idx in indices:
        ...
        for t in scheduler.timesteps:
            if hasattr(unet, "base_model"):
                noise_pred = unet.base_model(
                    sample=latents,
                    timestep=t,
                    encoder_hidden_states=prompt_joint,
                    return_dict=False,
                )[0]
            else:
                noise_pred = unet(
        ...
        imgs_joint = vae.decode(latents_final).sample
        joint_img = imgs_joint[0]

        # 拆分 joint -> CF/FA（压缩宽度）
        cf_small = joint_img[:, :, : SIZE // 2]
        fa_small = joint_img[:, :, SIZE // 2 :]

        # 插值回 512x512
        cf_full = F.interpolate(cf_small.unsqueeze(0), size=(SIZE, SIZE), mode="bilinear", align_corners=False)[0]
        fa_full = F.interpolate(fa_small.unsqueeze(0), size=(SIZE, SIZE), mode="bilinear", align_corners=False)[0]
...
@torch.no_grad()
def generate_pairs_from_data(unet, vae, tokenizer, text_encoder,
                             amount: int, out_dir: str, steps: int = 50,
                             strength: float = 0.6):
    """
    基于真实 CF-FA 对进行 img2img：先编码 joint，再在中间时间步加噪声后去噪，生成新 pair。
    ...
    num_t = len(timesteps)
    # strength=1.0 -> timesteps[0] (最大 t)，strength=0.0 -> timesteps[-1] (最小 t)
    ...
    # 带进度条的迭代器
    if tqdm is not None:
        data_iter = tqdm(loader, desc="基于真实对生成 Joint CF-FA 增强样本", ncols=80)
    else:
        data_iter = loader
    ...
    for batch in data_iter:
        ...
        # 构建 joint（与训练时一致），joint 范围 [-1,1]
        from train import build_joint_image  # 延迟导入避免循环引用问题
...
def main():
    parser = argparse.ArgumentParser(description="Joint CF-FA 生成推理脚本 v25")
    parser.add_argument(
        "-n", "--name", required=True,
        help="训练时使用的实验名称（与 0_train.py 的 --name 对应）",
    )
    parser.add_argument(
    ...
    if not os.path.isdir(lora_dir):
        raise FileNotFoundError(
            ...
            f"请确认已使用 v25/0_train.py 训练并保存 best_checkpoint。"
        )
    ...
    if args.mode == "noise":
        ...
        print("\n[3/3] 开始基于真实 CFFA 图像对加噪声再去噪生成新的 Joint CF-FA 图像对...")
