# -*- coding: utf-8 -*-
"""
Joint CF-FA Generation Training Script (v25)
-------------------------------------------
...
核心设计：
- 将一对配准好的 CF(左) 和 FA(右) 通过“宽度压缩 + 拼接”变成一张 512x512 的单图：
    - CF, FA 都从 512x512 双线性缩放到 512x256
    - 在宽度维度上拼接： joint = cat([CF_small, FA_small], dim=3) -> [B,3,512,512]
- 使用 SD1.5 的 VAE 对 joint 进行编码，UNet + LoRA 只对 joint 图像建模。
- 文本提示只用一条固定 prompt，训练目标为标准噪声 MSE + 可选 latent 高频 L1。

训练输出目录：
- /results/out_joint_sd15_cffa_pairs/{name}/
...
# Joint 生成模型输出根目录
OUT_ROOT = "/data/student/Fengjunming/SDXL_ControlNet/results/out_joint_sd15_cffa_pairs"
...
def build_joint_image(cf, fa):
    """
    将 CF, FA (B,3,512,512, [-1,1]) 压缩宽度并拼接成 joint (B,3,512,512).
    """
    # 插值目标尺寸 (H,W) = (SIZE, SIZE//2) = (512,256)
    cf_small = F.interpolate(cf, size=(SIZE, SIZE // 2), mode="bilinear", align_corners=False)
    fa_small = F.interpolate(fa, size=(SIZE, SIZE // 2), mode="bilinear", align_corners=False)
    joint = torch.cat([cf_small, fa_small], dim=3)  # 在宽度维度拼接
    return joint


def evaluate_joint(val_loader, vae, unet, noise_scheduler, tokenizer, text_encoder, args):
    """
    验证 Joint 生成模型：在固定时间步上评估噪声预测 MSE。
    """
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            prompt_embeds = get_joint_prompt_embeds(b, tokenizer, text_encoder)
...
def visualize_random_pairs(unet, vae, tokenizer, text_encoder,
    ...
    """
    从纯噪声生成若干 joint CF-FA 图像，用于训练过程可视化。
    每个样本保存为：
      pair_xx/cf.png
      pair_xx/fa.png
      pair_xx/joint.png
    ...
    for idx in range(num_samples):
        ...
        imgs_joint = vae.decode(latents_final).sample  # [1,3,512,512], [-1,1]
        joint_img = imgs_joint[0]

        # 拆分 joint -> CF/FA（仍是压缩宽度的 512x256）
...
def main():
    parser = argparse.ArgumentParser(description="Joint CF-FA 生成模型训练脚本 v25")
    parser.add_argument("-n", "--name", default="joint_cffa_v25")
