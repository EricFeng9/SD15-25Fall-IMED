...
基于预训练的vessel2img模型，使用黄斑区域的血管分割图生成黄斑区域CF图。

训练目标:
- 条件：黄斑区域血管分割图 (segA/pre{XX}Fundus_fake_B.png)
- 目标：黄斑区域CF图 (trainA/{XX}Fundus.jpg)

数据结构:
- 数据目录: CF_OCTA_origin/
  - segA/pre{XX}Fundus_fake_B.png    -> 条件 (黄斑区域血管分割图)
  - trainA/{XX}Fundus.jpg           -> GT (黄斑区域CF图)

特点:
- 基于已训练的vessel2img模型进行微调
- 使用黄斑区域的血管分割图作为条件
- 生成黄斑区域的CF图
- 共71对样本，训练集验证集划分比例为9:1
"""
...
# ============ 数据集配置 ============
# 黄斑区域血管分割图 -> 黄斑区域CF图 数据集 (CF_OCTA_origin)
DATA_CONFIG = {
    "crop_vessel": {
        "data_root": "/data/student/Fengjunming/diffusion_registration/SDXL_ControlNet/data/CF_OCTA_origin",
        "vessel_dir": "segA",           # 条件图目录：黄斑区域血管分割图
        "fundus_dir": "trainA",          # 目标图目录：黄斑区域CF图
        "total_samples": 71,              # 总样本数
        "train_ratio": 0.9,               # 训练集比例 9:1
    },
}

# 预训练模型路径
PRETRAINED_CHECKPOINT = "/data/student/Fengjunming/diffusion_registration/SDXL_ControlNet/results/out_ctrl_sd15_vessel2img/cf/260227_2_v29_2/best_checkpoint"
...
OUT_ROOT = "/data/student/Fengjunming/diffusion_registration/SDXL_ControlNet/results/out_ctrl_sd15_crop2crop_cf"
...
def remove_optic_disc(mask_np, vessel_threshold=0):
    """
    二值化血管图，将所有大于阈值的像素设为255（纯白），其余设为0（黑色）
    
    参数:
        mask_np: numpy数组，分割图（可能是RGB或灰度）
        vessel_threshold: 血管阈值，大于此值的像素设为255，其他设为0
...
class CropVesselDataset(Dataset):
    """
    黄斑区域血管分割图到黄斑区域CF图的数据集 (CF_OCTA_origin)
    条件：黄斑区域血管分割图 (segA/pre{XX}Fundus_fake_B.png)
    目标：黄斑区域CF图 (trainA/{XX}Fundus.jpg)
    """
    def __init__(self, data_config, split):
        ...
        self.vessel_dir = os.path.join(self.root_dir, data_config["vessel_dir"])
        self.fundus_dir = os.path.join(self.root_dir, data_config["fundus_dir"])
        self.split = split
        self.samples = []
        
        # 获取所有有效的样本ID
        # 文件名格式: segA/pre{XX}Fundus_fake_B.png, trainA/{XX}Fundus.jpg
        all_sample_ids = []
        
        # 遍历trainA目录获取所有CF图的样本ID
        for fname in sorted(os.listdir(self.fundus_dir)):
            if fname.endswith('.jpg') and 'Fundus' in fname:
                # 提取样本ID: 00Fundus.jpg -> 00
                sample_id = fname.replace('Fundus.jpg', '')
                vessel_fname = f"pre{sample_id}Fundus_fake_B.png"
                vessel_path = os.path.join(self.vessel_dir, vessel_fname)
                fundus_path = os.path.join(self.fundus_dir, fname)
        ...
        train_count = int(total * data_config["train_ratio"])
        
        if split == 'train':
            selected_ids = all_sample_ids[:train_count]
        else:  # val
            selected_ids = all_sample_ids[train_count:]
        
        # 构建样本列表
        for sample_id in selected_ids:
            vessel_path = os.path.join(self.vessel_dir, f"pre{sample_id}Fundus_fake_B.png")
            fundus_path = os.path.join(self.fundus_dir, f"{sample_id}Fundus.jpg")
        ...
        print(f"[Dataset] Found {len(self.samples)} pairs for {split} split (total: {total}, train: {train_count}, val: {total - train_count})")
    ...
    def __getitem__(self, idx):
        ...
        # 预处理：二值化血管图，所有大于0的像素设为255（纯白）
        vessel_np = np.array(vessel_pil)
        vessel_binary = remove_optic_disc(vessel_np, vessel_threshold=0)
        vessel_pil = Image.fromarray(vessel_binary).convert("RGB")
        
        # Resize到统一尺寸
        # 血管图使用NEAREST保持二值边界清晰，目标图使用BICUBIC保持质量
        vessel_pil = vessel_pil.resize((SIZE, SIZE), Image.NEAREST)
...
def evaluate(val_loader, vae, unet, cn_s, noise_scheduler, tokenizer, text_encoder, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            # 将生成图像转为tensor [0, 1]
            pred_tensor = transforms.ToTensor()(output_img).to(DEVICE)
            
            # 将GT从[-1,1]转到[0,1]
            tgt_tensor = (tgt.squeeze(0) + 1) / 2
            
            # 像素空间MSE
            loss = F.mse_loss(pred_tensor, tgt_tensor, reduction="mean")
...
def visualize_inference(val_loader, vae, unet, cn_s, noise_scheduler, tokenizer, text_encoder, args, step, out_dir):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            # 使用样本ID作为文件名
            name = os.path.basename(vessel_path[0]).replace("_vessel_binary.png", "")
            ...
            Image.fromarray(tgt_save).save(os.path.join(infer_dir, f"{name}_03_gt_cf_registered.png"))
...
def main():
    ...
    parser.add_argument("--guidance_scale", type=float, default=3.5, help="CFG guidance scale (默认3.5，与测试脚本一致)")
    ...
    print(f"[验证] Step 0 (训练前) | Pixel_MSE_Loss: {initial_val_loss:.6f}")
    ...
    while global_step <= args.max_steps:
        for batch in train_loader:
            ...
            if global_step % 500 == 0 and global_step > 0:
                ...
                val_msg = f"[验证] Step {global_step} | Pixel_MSE_Loss: {val_loss:.6f} | Best: {best_val_loss:.6f}"
                print(val_msg)
                ...
                if val_loss < best_val_loss:
                    ...
                    with open(os.path.join(best_dir, "best_checkpoint_info.txt"), "w", encoding="utf-8") as f:
                        ...
                        f.write(f"Validation Loss (Pixel_MSE): {val_loss:.6f}\n")
