# -*- coding: utf-8 -*-
"""
CF 生成模型训练脚本 (V22-CF-GEN)
--------------------------------

目标：
- 在 CFFA 数据集上训练一个仅生成 CF 图像的扩散模型（UNet + LoRA），不使用 ControlNet。
- 训练完成后，可以从纯噪声生成同域的 CF 图像，用于后续与 cf2fa 模型串联，扩增 CF-FA 成对数据。

设计要点：
- 数据：与 v22/train.py 一样，只使用 operation_pre_filtered_cffa_augmented (CFFADataset_v2)。
- 模型：SD1.5 的 VAE + UNet2DConditionModel + UNet LoRA（无 ControlNet）。
- 损失：沿用 v22 的 compute_total_loss（噪声 MSE + 可选 latent 高频 L1，通过 --hf_lambda 控制）。
- 验证：每 500 step，在 VAL_TIMESTEPS 固定时间步上评估噪声 MSE，记录到 validation_log.txt。
- 可视化：每次验证额外从随机噪声采样 10 张 CF 图像，保存到 step_XXXXXX_random_cf/pair_XX/{cf.png}。
- Checkpoint：
  - latest_checkpoints/step_XXXXXX/ 下滚动保留最近 3 个 checkpoint（Top-K 滚动保存）。
  - best_checkpoint/ 下保留验证集损失最优的一个 checkpoint。
"""
...
def get_cf_prompt_embeds(bs, tokenizer, text_encoder):
    """
    专门用于 CF 生成的 prompt embedding（固定为彩色眼底图）。
    """
...
def evaluate_cf(val_loader, vae, unet, noise_scheduler, tokenizer, text_encoder, args):
    ...
    with torch.no_grad():
        for batch in val_loader:
            cf, _, _, _ = batch  # CFFADataset_v2: (cond_tile=CF, tgt=FA, ...)
            cf = cf.to(DEVICE)
            b = cf.shape[0]

            latents = vae.encode(cf).latent_dist.sample() * vae.config.scaling_factor
            prompt_embeds = get_cf_prompt_embeds(b, tokenizer, text_encoder)
...
@torch.no_grad()
def visualize_random_cf(unet, vae, tokenizer, text_encoder,
...
def main():
    ...
    parser.add_argument("-n", "--name", default="cf_gen_v22")
    ...
    while global_step < args.max_steps:
        for batch in train_loader:
            ...
            if global_step % 500 == 0:
                ...
                print(f"[可视化] 在 {vis_dir} 生成 10 张随机 CF 图像...")
                visualize_random_cf(unet, vae, tokenizer, text_encoder, 10, vis_dir, 50)
