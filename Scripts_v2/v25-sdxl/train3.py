...
def visualize_random_pairs(unet, vae, tokenizer, tokenizer_2, text_encoder, text_encoder_2,
    ...
    """
    基于真实 CF-FA 对进行 img2img 生成，用于训练过程可视化（SDXL版本）。
    ...
    try:
        ...
        print(f"  - 将生成 {len(indices)} 个可视化样本")
    ...
    while global_step < args.max_steps:
        for batch in train_loader:
            ...
            if global_step % 500 == 0:
                ...
                # 可视化基于真实数据的 joint CF-FA 图像增强（img2img）
                vis_dir = os.path.join(out_dir, f"step_{global_step:06d}_random_pairs")
                print(f"[可视化] 在 {vis_dir} 基于真实数据生成 10 组 CF-FA 图像增强样本 (strength={args.vis_strength})...")
                visualize_random_pairs(
                    unet, vae, tokenizer, tokenizer_2, text_encoder, text_encoder_2, 10, vis_dir, 50, strength=args.vis_strength
                )
